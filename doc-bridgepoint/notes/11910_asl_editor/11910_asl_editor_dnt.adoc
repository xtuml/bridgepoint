...
x Add to the ActionDialect enum in the metamodel
  - TODO: make the same change in MC metamodel
...
- ExplorerView::handleOpen()
  ...
  - Why does it set the dialect of the element being opened on line 2824 if it's not masl|oal|none?  Don't understand
    why it's overwriting the user-set dialect value.
    - To reproduce: set dialect to ASL, then open the editor by double-clicking.  Dialect will be changed to workspace
    default (generally OAL).
