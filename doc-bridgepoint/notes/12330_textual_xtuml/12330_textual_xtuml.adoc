...
the model element tree, however parse roots are syntactically at the root within
...
quotes. There is no restriction on characters within extended names except
...
Action bodies are expected to be defined in files separate from the structural
...
receiving the particular event while in the particular state. A transition result
...
begin with the character 'R' followed by a positive integer:
