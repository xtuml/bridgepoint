...
Implementation of validation of the partial-path match must consider that full validation of a scoped_access rule which includes one or more scoped_path_segments can not be completed until the entire expression is parsed. This situation where validation has to be "delayed" until the entire expression is in-hand is new to the BridgePoint archetecture. Before this, grammar rules validated in OAL Validation utility functions always had enough information as each rule completed. To handle this situation where the entire scoped_access rule needs to be parsed before it is validated, a scoped_path_segment cache is introduced. 
...
The interitive form reused a code-pattern used in the previous implementation of Scoped_data_type_validate() (and other places in BridgePoint) that took advantage of the BridgePoint archecture's search facility that uses {C_C | PE_PE | EP_PKG}.collectVisibleElementsForName.  

The interitive solution was simpler and was thus the appraoch taken. With this approach a decision was made to abondon the extra collection that the recursive implementation has used to store matches in favor of allowing the matching routine to return a set of matches. This again greatly simplied the solution but caused some "archetuctural" changes. For example, references to the datatypes used were published and MC-Java was updated as needed to support the published references.
