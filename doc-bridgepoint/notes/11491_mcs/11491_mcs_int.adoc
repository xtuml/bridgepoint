...
attempted. Some steps are marked "optional". The BridgePoint team recommends
following these steps for a cleaner project environment, however they are not
strictly necessary.

. Assure BridgePoint is not running
. In the project folder on disk:
  .. Delete `.externalToolBuilders/`
  .. Delete `Debug/` and `Release/` if they exist (optional)
  .. Delete `gen/code_generation/` if it exists (optional)
  .. Delete `src/` if it exists (optional)
  .. Delete `.cproject` if it exists
. In the `.project` file:
  .. Delete the entire `<buildSpec>` section
  .. In the `<natures>` section, delete all entries except
     `<nature>org.xtuml.bp.core.xtumlnature</nature>`
. Open BridgePoint
. Use the "BridgePoint Utilities > Set Model Compilers" menu to choose the model
  compilers you wish to be enabled. For MC-3020 projects, use this wizard to
  enable the CDT if desired.
. If other builders were used in your project, restore them from your backup
  files
...
. [[dr-3]] https://support.onefact.net/issues/10636[10636 - Update build server with new host version of BP]
. [[dr-4]] https://support.onefact.net/issues/10637[10637 - Upgrade welcome plugin projects]
