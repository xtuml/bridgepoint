...
- Look into tuning JVM, params like -Xms, 
- Look into tuning GC, is it running too much?
- Look for memory leaks, profile BridgePoint during the life and look through the collected memory data.  Perform deletions and watch the memory size assuring garbage collection occurs.  Also perform operations that close views/editors where temporary data should be garbage collected.
- Look into UseParallelGC (garbage collects with multiprocessors)
