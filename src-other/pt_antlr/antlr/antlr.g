// This version of antlr.g does not build ast's for
// any rules but the ebnf.bnf grammar.
header {
package antlr;
}
 ...
 *
 * $Id: antlr.g,v 1.3 2006/03/29 19:44:15 jeremyr Exp $
 * Id: //depot/code/org.antlr/release/antlr-2.7.2/antlr/antlr.g#1 
 */
...

{
    ...
    private static int idGrammar = 0;
    private static int idRule = 0;
    private static int idId = 0;
    private static int idBlock = 0;
    private static int idAlternative = 0;
    private static int idElement = 0;
    private static int idRange = 0;
    private static int idTerminal = 0;
    private static int idEbnf = 0;
...
element
	:	el:elementNoOptionSpec (elementOptionSpec)?
	;
...
[ Token label ]
{
	...
	:	crLeft:CHAR_LITERAL RANGE crRight:CHAR_LITERAL
		...
		{ behavior.refCharRange(crLeft, crRight, label, autoGen, lastInRule());
          #range = #([Range,"Range"], crLeft, crRight);
        }
	|
		...
		{ behavior.refTokenRange(trLeft, trRight, label, autoGen, lastInRule()); 
          #range = #([Range,"Range"], [trLeft], [trRight]);
        }
	;
