 ...
 *
 * $Id: Tool.java,v 1.2 2003/07/10 17:52:37 greg Exp $
...
public class Tool {
	...
	protected static String sqlFileName = "";
    ...
    public int doEverything(String[] args) {
        ...
        try {
            ...
            p.setASTFactory(f);
            p.setASTNodeClass("antlr.PTPopulatorAST");
            BaseAST.setVerboseStringConversion(true, p.getTokenNames());
            ...
            PTPopulatorAST t = (PTPopulatorAST)p.getAST();
            if (t != null) {
                // SQL serialize the tree, showing
                // different physical node class types
				FileOutputStream sqlFile = new FileOutputStream( sqlFileName );
				PTPopulatorAST.sqlWriter = new OutputStreamWriter(sqlFile);
                try{
                    t.sqlSerialize();
                    
					PTPopulatorAST.sqlWriter.write("\n");
					PTPopulatorAST.sqlWriter.flush();
					sqlFile.close();
					PTPopulatorAST.sqlWriter = null;
                }
                ...
                System.out.print("AST pointer is null!\n");
                System.out.flush();
            }

			System.out.print("File " + sqlFileName + " generated.\n");
			...
			fatalError("FileNotFoundException: " + fnf.getMessage());
		}
...
public static Object factory2 (String p, Object[] initargs) {
    ...
    private static void help() {
		...
		System.err.println("  -sql=filePath      specify file name for sql output.");
		System.err.println("  -nocodegen         don't generate the parser and token files.");
    ...
    protected void processArguments(String[] args) {
        for (int i = 0; i < args.length; i++) {
			...
			else if (args[i].equals("-nocodegen")) {
				generateCode = false;
				setArgOK(i);
			}
