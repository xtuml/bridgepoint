...
| Java Virtual Machine  | Version >= 17
...
During the testing of BridgePoint v7.8, problems were uncovered with cut/paste
(model element move) on Windows. While this issue persists, it is recommended
to avoid using cut/paste where copy/paste is sufficient and to always operate
within a version controlled environment to avoid losing work. More information
on the issue can be found link:https://support.onefact.net/issues/12955[here].
