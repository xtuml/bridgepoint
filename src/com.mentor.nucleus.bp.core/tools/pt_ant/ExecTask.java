...
public class ExecTask extends Task {
	...
	protected final void runExecute(Execute exe) throws IOException {
		...
		if (!spawn) {
			File timeFile = new File(project.getBaseDir().toString() + "/" + project.getName() + "_exec_times.txt" );
			BufferedWriter out = new BufferedWriter(new FileWriter(timeFile, true));
			...
			long executeTime = (timeAfter - time);
			long timeToSet = (executeTime + 30000);
			
			String timeToSetString = Long.toString(timeToSet);
			
			out.write(timeToSetString + "\n" );
