...
tagname="${1}"
bugnum="${2}"
svnroot="svn://tucson.wv.mentorg.com/arch1/products/tiger/doc_repository"
...
if [ "${bugnum}" == "" ]; then
  echo -e "\n\tERROR: You must give a bug number as the second argument to the program.\n"
  exit
fi

echo -e "\n\tUsing tag: ${tagname}, bug number: ${bugnum}\n"

echo -e "Logging in to CVS as user 'build'"
export CVSROOT=":pserver:build@tucson.wv.mentorg.com:/arch1/products/tiger/repository"

cvs rtag -F ${tagname} Documentation
cvs rtag -F ${tagname} MC-Documentation
cvs rtag -F ${tagname} MC-Java
cvs rtag -F ${tagname} MC-Java.test
cvs rtag -F ${tagname} Models
cvs rtag -F ${tagname} Models_SF
...
echo -e "Tagging projects in SVN"
svn mkdir -m "Job:${bugnum}
Making tag directory" --username build --password initial ${svnroot}/tags/${tagname}

svn copy -m "Job:${bugnum}
Tagging Models project with ${tagname}." --username build --password initial \
    ${svnroot}/trunk/Models ${svnroot}/tags/${tagname}/Models
