...
.// Modified:  $Date: 2013/05/10 06:09:43 $
.//
...
// Generated by ${info.arch_file_name}, $$Revision: 1.111 $$
//
// (c) Copyright 2005-2014 by Mentor Graphics Corp.  All rights reserved.
//
//====================================================================
  ...
  .for each event in events
    ...
    .for each STTEntry in STTEntries
      ...
      .if (not_empty newStateTxn)
         ...
         .select any actionblock from instances of ACT_ACT where ((selected.Action_Id == "${action.Act_ID}") AND (selected.Type == "SM"))
...
.include "color/${gdn_result.body}_package_spec.clr"
...
${bridge.Action_Semantics}
      ...
      ${package.application_root_class}.log.println(ILogger.BRIDGE, "${bridge.Name}", " Bridge entered: $cr{ee.Name}::$cr{bridge.Name}") ; 
        ...
        .select any actionblock from instances of ACT_ACT where ((selected.Action_Id == "${bridge.Brg_ID}") AND (selected.Type == "BR"))
        .select any block related by actionblock->ACT_BLK[R666]
        .invoke blck = blck_xlate(block)
${blck.body}
      .end if
  ...
  .//
.end if 
.include "color/${gdn_result.body}_import_spec.clr"
...
${dbattr.Action_Semantics}
                .else
                  .// Generate Action Language Code using Processing Subsystem Instances
                  .select any actionblock from instances of ACT_ACT where ((selected.Action_Id == "${attribute.Attr_ID}") AND (selected.Type == "AT"))
                  .select any block related by actionblock->ACT_BLK[R666]
                  .invoke blck = blck_xlate(block)
ModelRoot modelRoot = getModelRoot();
${blck.body}
...
${transform.Action_Semantics}
            .else
              ...
              .end if
              .// Generate Action Language Code using Processing Subsystem Instances
              .select any actionblock from instances of ACT_ACT where ((selected.Action_Id == "${transform.Tfr_ID}") AND ((selected.Type == "CO") OR (selected.type == "IO")))
              .select any block related by actionblock->ACT_BLK[R666]
              .invoke blck = blck_xlate(block)
${blck.body}
            .end if
...
${function.Action_Semantics}
    .else
      ...
      .select any actionblock from instances of ACT_ACT where ((selected.Action_Id == "${function.Sync_ID}") and (selected.Type == "FN"))
      .select any block related by actionblock->ACT_BLK[R666]
      .invoke blck = blck_xlate(block)
${blck.body}
    .end if
  ...
  .include "color/${gdn_result.body}_startspec.clr"
