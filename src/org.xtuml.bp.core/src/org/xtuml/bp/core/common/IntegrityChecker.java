...
public class IntegrityChecker {
	...
	public static IntegrityIssue_c[] runIntegrityCheck(final NonRootModelElement element,
		...
		if (integrityCheckIsEnabled(system, ignoreProjectPreference)) {
	...
	private static void createIntegrityIssues(NonRootModelElement element) {
		...
		if (integrityCheckIsEnabled(system, false)) {
	...
	public static void startIntegrityChecker(HashSet<PersistableModelComponent> persisted) {
		...
		if (integrityCheckIsEnabled(sys, false)) {
	 ...
	 * 
	 * @param system
	 *            The system to use to check the project preference. If this is
	 *            null we will use the workspace preference.
	 ...
	 */
	private static boolean integrityCheckIsEnabled(SystemModel_c system, boolean ignorePreference) {
		boolean isEnabled = true;
		if (!ignorePreference) {
			IPreferenceStore store = CorePlugin.getDefault().getPreferenceStore();
			isEnabled = store.getBoolean(BridgePointPreferencesStore.ENABLE_MODEL_INTEGRITY_CHECK);
			// If we don't have the system name then we'll use the workspace
			// preference. If we
			// do have it then we can use the project preference
			if (system != null) {
				isEnabled = BridgePointProjectModelIntegrityPreferences
						.getProjectBoolean(BridgePointPreferencesStore.ENABLE_MODEL_INTEGRITY_CHECK, system.getName());
			}
		}
		return isEnabled;
	}
