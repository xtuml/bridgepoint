...
public class XMIImportPage extends WizardDataTransferPage {
	...
	protected XMIImportPage(String name, IStructuredSelection structuredSelection) {
		super(name);
		setDescription("Select an XMI file for import");
		setTitle(name);
	}
	...
	protected boolean validateSourceGroup() {
		...
		if(sourceFilePath.getFileExtension() == null) {
			return invalidXmiFile();
		}
		if(XMITranslate.supportedExtensions.contains(sourceFilePath.getFileExtension())) {
			return invalidXmiFile();
		}
	...
	private boolean invalidXmiFile() {
		setMessage("The given path is not to a valid xmi file.", DialogPage.INFORMATION);
		return false;
	}
	...
	private void handleSourceBrowseButtonPressed() {
		...
		dialog.setFilterExtensions(XMITranslate.supportedExtensions.toArray(new String[0]));
	...
	String fPageName = "XMI Import";
	protected final String STORE_SOURCE_NAMES= fPageName + ".XMI"; //$NON-NLS-1$
