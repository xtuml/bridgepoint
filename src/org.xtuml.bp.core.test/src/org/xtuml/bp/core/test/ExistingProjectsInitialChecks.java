...
public class ExistingProjectsInitialChecks extends CanvasTest {
	...
	/*	public void testNewShapeAfterRestartOnSequenceDiagram() throws PartInitException {
		...
		public void testEditorsRemainOpenAfterClose() {
			...
			assertNotNull("Editor, TestClass1: Model Class Description, did not correctly restore",
		...
		public void testPlacholderInstances() {
			...
			Class clazz = SetupCreationTests.loadClassFromPlugin("org.xtuml.bp.ui.text.test", "UITextTest"); //$NON-NLS-2$ //$NON-NLS-1$
			IFile objFile = SetupCreationTests.callGetExistingPlaceHolderFromManager(clazz, obj,
					DescriptionEditorInputFactory.PLACEHOLDER_EXTENSION);
			assertNotNull(objFile);
			IFile opFile = SetupCreationTests.callGetExistingPlaceHolderFromManager(clazz, op,
		...
		public void testInterfaceAssignmentInterfaceContainedInDifferentPackageRoot() {
			...
			Package_c containingPackage = Package_c.getOneEP_PKGOnR1405(system, new ClassQueryInterface_c() {
				public boolean evaluate(Object candidate) {
					return ((Package_c) candidate).getName().equals("InterfaceAssignmentSetup-2");
				}
			});
			assertNotNull("Unable to locate containing package.", containingPackage);
	
			Package_c compPkg = Package_c.PackageInstance(containingPackage.getModelRoot(), new ClassQueryInterface_c() {
				public boolean evaluate(Object candidate) {
					return ((Package_c) candidate).getName().equals("Component Package");
				}
			});
	
			assertNotNull("Unable to location component package for interface reference.", compPkg);
	
			Component_c comp = Component_c.getOneC_COnR8001(PackageableElement_c.getManyPE_PEsOnR8000(compPkg));
			assertNotNull("Unable to location component which the interface reference connects to.", comp);
	
			Provision_c provision = Provision_c
					.getOneC_POnR4009(InterfaceReference_c.getOneC_IROnR4016(Port_c.getOneC_POOnR4010(comp)));
			...
			GraphicalEditor ce = UITestingUtilities.addElementToGraphicalSelection(provision);
			assertTrue("Formalize was not available.",
					UITestingUtilities.checkItemStatusInContextMenu(ce.getCanvas().getMenu(), "Formalize...", "", false));
	
			// this selection is done after the check above
			// to test that loading properly occurs
			Interface_c iface = Interface_c.getOneC_IOnR8001(
					PackageableElement_c.getManyPE_PEsOnR8000(Package_c.getManyEP_PKGsOnR1405(system)),
					new ClassQueryInterface_c() {
						public boolean evaluate(Object candidate) {
							return ((Interface_c) candidate).getName().equals("InterfaceAssignmentSetup-1");
						}
					});
		...
		public void testComponentAssignmentComponentContainedInDifferentPackageRoot() {
			...
			Package_c containingPackage = Package_c.getOneEP_PKGOnR1405(system, new ClassQueryInterface_c() {
				public boolean evaluate(Object candidate) {
					return ((Package_c) candidate).getName().equals("ComponentAssignmentSetup-2");
				}
			});
			assertNotNull("Unable to locate containing package.", containingPackage);
			...
			Package_c compPkg = Package_c.PackageInstance(containingPackage.getModelRoot(), new ClassQueryInterface_c() {
				public boolean evaluate(Object candidate) {
					return ((Package_c) candidate).getName().equals("Component Reference Package");
				}
			});
			assertNotNull("Unable to location component package for interface reference.", compPkg);
	
			ComponentReference_c icomp = ComponentReference_c
					.getOneCL_ICOnR8001(PackageableElement_c.getManyPE_PEsOnR8000(Package_c.getManyEP_PKGsOnR1405(system)));
			assertNotNull("Unable to location component which the interface reference connects to.", icomp);
	
			CanvasUtilities.openCanvasEditor(compPkg);
			UITestingUtilities.clearGraphicalSelection();
	
			GraphicalEditor ce = UITestingUtilities.addElementToGraphicalSelection(icomp);
			assertTrue("Assign Component was not available.", UITestingUtilities
					.checkItemStatusInContextMenu(ce.getCanvas().getMenu(), "Assign Component...", "", false));
			...
			Component_c component = Component_c.getOneC_COnR8001(
					...
					new ClassQueryInterface_c() {
						...
						public boolean evaluate(Object candidate) {
							return ((Component_c) candidate).getName().equals("ComponentAssignmentSetup-1");
						}
		...
		public void testDataTypeAssignmentInterfaceOperationInDifferentPackageRoot() {
			...
			InterfaceOperation_c io = InterfaceOperation_c.getOneC_IOOnR4004(ExecutableProperty_c.getOneC_EPOnR4003(iface));
			...
			OperationsC_IOPropertySource propertySource = new OperationsC_IOPropertySource(io);
			int result = -1;
			...
			//jj
			assertTrue("Data Type defined in another package is not available for assignment.", result != -1);
			propertySource.setPropertyValue("DataType", "DataTypeAssignmentSetup");
			DataType_c dt = DataType_c.getOneS_DTOnR4008(io);
			assertNotNull(dt);
			assertTrue("Data Type defined in another package was not allowed as return type.",
		...
		public void testDataTypeAssignmentPropertyParameterInDifferentPackageRoot() {
			...
			io.Newparameter();
			PropertyParameter_c pp = PropertyParameter_c.getOneC_PPOnR4006(ExecutableProperty_c.getOneC_EPOnR4003(iface));
			...
			ParametersC_PPPropertySource propertySource = new ParametersC_PPPropertySource(pp);
			int result = -1;
			IPropertyDescriptor[] propertyDescriptors = propertySource.getPropertyDescriptors();
			for (int i = 0; i < propertyDescriptors.length; i++) {
			...
			DataType_c dt = DataType_c.getOneS_DTOnR4008(io);
			assertNotNull(dt);
			assertTrue("Data Type defined in another package was not allowed as parameter type.",
					dt.getName().equals("DataTypeAssignmentSetup"));
		}
		...
		public void testUseCasePackageOnRestart() {
			...
			assertNotNull("Use Case package was not successfully loaded.", usecasePackage);
