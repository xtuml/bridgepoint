package org.xtuml.bp.io.image.generator;
...
public class Generator extends Task {
    ...
    public static void genAll(final SystemModel_c sys) {
        genAll(sys, null, new Path("doc"));
    }

    public static void genAll(final SystemModel_c sys, IProject destinationProject, IPath outputLocation) {
        ...
        for (int i = 0; i < ooags.length; i++) {
            ...
            for (NonRootModelElement nrModel : models) {
                ...
                if (model.getRepresents() != null) {
                    ModelRoot mr = ((NonRootModelElement) model.getRepresents()).getModelRoot();
                    String canvasName = GraphicsUtil.getCanvasEditorTitle((NonRootModelElement) model.getRepresents());
                    if ((mr.getId().startsWith(rootPath)) || canvasName.equals(project.getName() + SMPD_NAME)) {
                        try {
                            createImage(model, Ooaofooa.getInstance(mr.getId()), destinationProject, outputLocation);
                        } catch (Throwable e) {
                            showWarning = true;
                            warningMessage += "\n\t" + canvasName;
    ...
    public static void run(Component_c rootElement) {
        ...
        createImage(ptCanvas, Ooaofooa.getInstance(root.getId()), null, new Path("doc"));
    }

    private static void createImage(final Model_c ptCanvas, final Ooaofooa modelRoot, final IProject destinationProject,
        ...
        Display.getDefault().syncExec(new Runnable() {
            ...
            public void run() {
                ...
                String prefFont = PlatformUI.getWorkbench().getPreferenceStore().getString("org.xtuml.bp.canvas.font"); //$NON-NLS-1$
                ...
                Rectangle extentRectangle = GraphicalZoomManager
                        .getExtentRectangle(GraphicalEditor.getAllSymbols(viewer, ptCanvas.Hascontainersymbol()));
                ...
                if (elements.length == 0) {
                    ...
                    Image img = new Image(Display.getDefault(), extentRectangle.width, extentRectangle.height);
                    PrintDiagramOperation.printImage(img, viewer, extentRectangle, ptCanvas.Hascontainersymbol(),
                ...
                try {
                    ...
                    path = path.append(outputLocation); // $NON-NLS-1$
                    ...
                    String containerName = GraphicsUtil.getContainerList(ptCanvas, modelRoot);
                    String fileNameBody = GraphicsUtil
                            .getCanvasEditorTitle((NonRootModelElement) ptCanvas.getRepresents())
                            + GraphicsUtil.getCanvasEditorPostTitle((NonRootModelElement) ptCanvas.getRepresents(),
                                    true);
                    fileNameBody = fileNameBody.replace(": ", "-"); //$NON-NLS-1$ //$NON-NLS-2$
                    if (!containerName.isEmpty()) {
                        fileNameBody = containerName + fileNameBody;
                    }
                    fullPath = path.append(fileNameBody + ".png").toString(); //$NON-NLS-1$
        ...
        try {
            ...
            if (plugin == null) {
                RuntimeException rtException = new RuntimeException(
                        "Unable to acquire the bp.io.image.Activator, therefore the image " + name
                                + " can not be loaded.");
                throw rtException;
            }
