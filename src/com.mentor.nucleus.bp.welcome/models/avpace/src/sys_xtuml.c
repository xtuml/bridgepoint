...
Escher_Extent_t * const * const domain_class_info[ SYSTEM_DOMAIN_COUNT ] = {
  &heart_class_info[0],
  &hostmonitor_class_info[0],
  &pacemaker_class_info[0]
};
...
typedef union {
  ...
  heart_DomainEvents_u mc_events_in_domain_heart;
  hostmonitor_DomainEvents_u mc_events_in_domain_hostmonitor;
  pacemaker_DomainEvents_u mc_events_in_domain_pacemaker;
...
static void ooa_loop( void )
{
  ...
  static const EventTaker_t * DomainClassDispatcherTable[ 3 ] =
    {
      heart_EventDispatcher,
      hostmonitor_EventDispatcher,
      pacemaker_EventDispatcher
    };
  Escher_xtUMLEvent_t * event;
