...
public class GraphicsEditorListener extends ModelChangeAdapter implements ITransactionListener {	
	 ...
	 * (non-Javadoc)
	 * 
	 * @seeorg.xtuml.bp.core.common.IModelChangeListener#
	...
	public void modelElementReloaded(ModelChangedEvent event) {
		if (m_editor != null && m_editor.getCanvas() != null && !m_editor.getCanvas().isDisposed()) {
			...
			// our model
			Object editorData = m_editor.getModel().getRepresents();
			if (event.getModelElement() != null && event.getNewModelElement() != null && (event.getModelElement().equals(editorData) || (event.getModelElement() instanceof Model_c
