package org.xtuml.bp.debug.java.access;
...
import org.xtuml.bp.core.MessageValue_c;
...
import org.xtuml.bp.core.ValueInStackFrame_c;
...
public class VerifierInvocationHandler implements InvocationHandler {
	...
	@Override
	public synchronized Object invoke(Object arg0, Method arg1, Object[] arg2) throws Throwable {
		...
		String ifaceName = className.substring(0, lPortTypePosn).replaceFirst("I", "");
		Component_c comp = Component_c.getOneC_COnR2955(component);
		if (comp == null) {
			comp = Component_c.getOneC_COnR4201(ComponentReference_c.getOneCL_ICOnR2963(component));
		}
		InterfaceReference_c[] ifRefs = InterfaceReference_c.getManyC_IRsOnR4016(port);
		...
		ExecutableProperty_c[] exProps = ExecutableProperty_c.getManyC_EPsOnR4003(iface);
		...
		if (arg2[0] instanceof ComponentInstance_c) {
			specifiedComponentInstance = ((ComponentInstance_c) arg2[0]).getExecution_engine_id();
		}
		if (comp != null) {
			java.util.UUID channelID = ComponentInstance_c.Resolvetargetchannel(comp.getModelRoot(), ifRef.getId(),
					targetInterfaceIsRequired, component.getExecution_engine_id(), specifiedComponentInstance);
			if (channelID != Gd_c.Null_unique_id()) {
				RuntimeChannel_c channel = (RuntimeChannel_c) component.getModelRoot()
						.getInstanceList(RuntimeChannel_c.class).getGlobal(channelID);
				ComponentInstance_c targetEngine = ComponentInstance_c.getOneI_EXEOnR2968IsInterfaceRequirerOf(channel);
				if (targetInterfaceIsRequired) {
					targetEngine = ComponentInstance_c.getOneI_EXEOnR2968IsInterfaceProviderTo(channel);
				}
				Stack_c targetStack = Stack_c.getOneI_STACKOnR2930(targetEngine);
				java.util.UUID bodyID = ComponentInstance_c.Resolvetargetbody(comp.getModelRoot(), channelID,
						"User realized call to " + methName, targetInterfaceIsRequired, methName);
								...
								if (shouldBlock) {
									...
									while (StackFrame_c.getOneI_STFOnR2965BlockedBy(localStackFrame) != null
										...
										synchronized (component) {
											...
											if (BPThread.qdResultsReady(localStack)) {
												...
												for (StackFrame_c resultSF : resultSFs) {
													if (resultSF == newStackFrame
														...
														if (localVsf != null) {
															...
															DataType_c dt = DataType_c.getOneS_DTOnR3307(resultRtv);
															InterfaceOperation_c ifOp = InterfaceOperation_c
																	.getOneC_IOOnR4004(exProp);
															if (ifOp != null) {
	...
	private void disposeTransientValueVariables(RuntimeValue_c rtVal) {
		...
		Value_c val = Value_c.getOneV_VALOnR801(TransientValueReference_c.getOneV_TVLOnR805(var));
	...
	private static Object convertToRawJavaType(Object toConvert, DataType_c dt, boolean isByRef,
		...
		if (name.equals("integer")) {
			try {
				...
				LOG.LogInfo("Unexpected integer format found converting Verifier data for realized invocation. "
	...
	private static RuntimeValue_c marshallValueIn(PropertyParameter_c propertyParameterC, Object value, UUID msgid,
		...
		Dimensions_c[] dims = Dimensions_c.getManyS_DIMsOnR4017(propertyParameterC);
		RuntimeValue_c rootValue = marshallContentIn(root, null, value, dt, dims.length, ci);
		StackFrame_c lclStackFrame = (StackFrame_c) root.getInstanceList(StackFrame_c.class).getGlobal(stackFrameId);
		BlockInStackFrame_c bsf = BlockInStackFrame_c.getOneI_BSFOnR2923(lclStackFrame);
		if (rootValue != null) {
			...
			var.setName("User realized value for " + propertyParameterC.getName());
			...
			ComponentInstance_c.Transfervaluetotarget(root, arg.getValue_id(), bodyID, stackFrameId, msgid, sf);
	...
	private static RuntimeValue_c marshallContentIn(ModelRoot root, RuntimeValue_c rootValue, Object value,
			DataType_c dt, int arrayDepth, ComponentInstance_c ci) {
		RuntimeValue_c result = null;
		if (rootValue != null) {
			result = rootValue;
		}
		if (arrayDepth > 0) {
			result = marshallArrayValueIn(root, rootValue, value, dt, arrayDepth, ci);
		} else {
			...
			if (cdt != null) {
				...
				result = marshallStructuredValueIn(root, rootValue, value, dt, result, ci);
			} else if (edt != null) {
	...
	private static RuntimeValue_c marshallEnumeratedValueIn(Object value, DataType_c dt, RuntimeValue_c result,
			DataType_c rootType) {
	...
	private static RuntimeValue_c marshallStructuredValueIn(ModelRoot root, RuntimeValue_c rootValue, Object value,
			DataType_c adt, RuntimeValue_c result, ComponentInstance_c ci) {
		...
		for (StructureMember_c strMember : members) {
			...
			if (meth != null) {
				...
				try {
					...
					try {
						...
						String ex = e.getLocalizedMessage();
						LOG.LogInfo(
								"Illegal argument exception while accessing field of realized structured data type: "
										+ ex);
					} catch (IllegalAccessException e) {
						String ex = e.getLocalizedMessage();
						LOG.LogInfo("Illegal access exception while accessing field of realized structured data type: "
								+ ex);
					}
			...
			if (javaMember != null) {
				ValueInStructure_c[] viss = ValueInStructure_c.getManyRV_VISsOnR3301(strVal);
	...
	private static RuntimeValue_c marshallCoreValueIn(ModelRoot root, RuntimeValue_c rootValue, Object value,
			DataType_c dt, ComponentInstance_c ci) {
		RuntimeValue_c result = null;
		if (rootValue != null) {
			result = rootValue;
		}
		...
		if (udt != null) {
			...
			if (realizedType != null) {
				...
				for (Method meth : methods) {
					if (meth.getName().equals("getValue") && meth.getParameterTypes().length == 0) {
						try {
							result = new RuntimeValue_c(root);
							...
							// TODO When dts0100910433 is promoted convert these
							// to Runtime exceptions
							LOG.LogInfo("Illegal argument exception converting realized data for Verifier invocation. "
									+ iae.getLocalizedMessage());
						} catch (IllegalAccessException iae) {
							LOG.LogInfo("Illegal access exception converting realized data for Verifier invocation. "
									+ iae.getLocalizedMessage());
						} catch (InvocationTargetException ite) {
							LOG.LogInfo("Invocation target exception converting realized data for Verifier invocation. "
					...
					for (Field field : fields) {
						if (field.getName().equals("value")) {
							try {
								...
								String ex = e.getLocalizedMessage();
								LOG.LogInfo("Illegal argument exception while accessing realized structured data type: "
										+ ex);
							} catch (IllegalAccessException e) {
								String ex = e.getLocalizedMessage();
								LOG.LogInfo("Illegal access exception while accessing realized structured data type: "
										+ ex);
							}
							break;
						}
	...
	public static RuntimeValue_c marshallArrayValueIn(ModelRoot root, RuntimeValue_c rootValue, Object value,
			DataType_c dt, int arrayDepth, ComponentInstance_c ci) {
		RuntimeValue_c result = null;
		if (rootValue != null) {
			result = rootValue;
		...
		if (value instanceof List<?>) {
			...
			for (Object content : (List<?>) value) {
				ValueInArray_c[] vias = ValueInArray_c.getManyRV_VIAsOnR3302(arrayVal);
				...
				member = marshallContentIn(root, member, content, dt, arrayDepth - 1, ci);
	...
	public static void executeRealizedCode(Stack_c stack) {
		...
		if (comp == null) {
			comp = Component_c.getOneC_COnR4201(ComponentReference_c.getOneCL_ICOnR2963(fee));
		}
		if (comp != null) {
			...
			if (success) {
				...
				synchronized (fee) {
					if (BPThread.qdFramesReady(stack)) {
						stack.Dequeue(false);
						StackFrame_c localStackFrame = StackFrame_c.getOneI_STFOnR2929(stack);
						Body_c body = Body_c.getOneACT_ACTOnR601(Block_c
								.getManyACT_BLKsOnR2923(BlockInStackFrame_c.getOneI_BSFOnR2923(localStackFrame)));
						StackFrame_c blockedFrame = StackFrame_c.getOneI_STFOnR2965Blocks(localStackFrame);
						ComponentInstance_c sender = ComponentInstance_c
								.getOneI_EXEOnR2930(Stack_c.getOneI_STACKOnR2943(blockedFrame));
						...
						if (val != null) {
							isOperation = true;
							MessageValue_c mv = MessageValue_c.getOneV_MSVOnR801(val);
							ProvidedExecutableProperty_c pep = ProvidedExecutableProperty_c.getOneSPR_PEPOnR841(mv);
							RequiredExecutableProperty_c rep = RequiredExecutableProperty_c.getOneSPR_REPOnR845(mv);
							if (pep != null) { // Provided Operation
								provOp = ProvidedOperation_c.getOneSPR_POOnR4503(pep);
								messageName = provOp.getName();
								ep = ExecutableProperty_c.getOneC_EPOnR4501(pep);
								Dimensions_c[] dims = Dimensions_c
										.getManyS_DIMsOnR4018(InterfaceOperation_c.getOneC_IOOnR4004(ep));
								returnArrayDepth = dims.length;
							} else { // Required Operation
								reqOp = RequiredOperation_c.getOneSPR_ROOnR4502(rep);
							...
							reqOp = RequiredOperation_c
									.getOneSPR_ROOnR685(RequiredOperationBody_c.getManyACT_ROBsOnR698(body));
							provOp = ProvidedOperation_c
									.getOneSPR_POOnR687(ProvidedOperationBody_c.getOneACT_POBOnR698(body));
							RequiredSignal_c reqSig = RequiredSignal_c
									.getOneSPR_RSOnR684(RequiredSignalBody_c.getOneACT_RSBOnR698(body));
							ProvidedSignal_c provSig = ProvidedSignal_c
									.getOneSPR_PSOnR686(ProvidedSignalBody_c.getOneACT_PSBOnR698(body));
							if (reqOp != null) {
								isOperation = true;
								messageName = reqOp.getName();
								ep = ExecutableProperty_c
										.getOneC_EPOnR4500(RequiredExecutableProperty_c.getOneSPR_REPOnR4502(reqOp));
							} else if (provOp != null) {
								isOperation = true;
								messageName = provOp.getName();
								ep = ExecutableProperty_c
										.getOneC_EPOnR4501(ProvidedExecutableProperty_c.getOneSPR_PEPOnR4503(provOp));
							} else if (reqSig != null) {
								messageName = reqSig.getName();
								ep = ExecutableProperty_c
										.getOneC_EPOnR4500(RequiredExecutableProperty_c.getOneSPR_REPOnR4502(reqSig));
							} else if (provSig != null) {
								messageName = provSig.getName();
								ep = ExecutableProperty_c
										.getOneC_EPOnR4501(ProvidedExecutableProperty_c.getOneSPR_PEPOnR4503(provSig));
							}
						}
						ComponentInstance_c ci = ComponentInstance_c.getOneI_EXEOnR2930(stack);
						if (ep != null) {
							PropertyParameter_c[] pParms = PropertyParameter_c.getManyC_PPsOnR4006(ep);
							PropertyParameter_c cursor = null;
							for (PropertyParameter_c parm : pParms) {
								PropertyParameter_c succeeds = PropertyParameter_c.getOneC_PPOnR4021Succeeds(parm);
								if (succeeds == null) {
									cursor = parm;
									break;
								}
							...
							for (RuntimeValue_c rtVal : argMap.keySet()) {
								marshallContentIn(rtVal.getModelRoot(), rtVal, argMap.get(rtVal).getValue(),
										DataType_c.getOneS_DTOnR3307(rtVal), argMap.get(rtVal).getArrayDepth(), ci);
							}
						} else {
						...
						if (ci != null) { // If component instance is null we're
							...
							if (isOperation) {
								...
								StackFrame_c topStFr = StackFrame_c.getOneI_STFOnR2929(stack);
								...
								if (resultValue != null) {
									...
									// Verifier
									ValueInStackFrame_c localVsf = ValueInStackFrame_c
											.getOneI_VSFOnR2951(localStackFrame);
									if (localVsf == null) {
										localVsf = new ValueInStackFrame_c(localStackFrame.getModelRoot());
										localVsf.relateAcrossR2951To(localStackFrame);
									}
									DataType_c dt = DataType_c
											.getOneS_DTOnR820(Value_c.getOneV_VALOnR2978(resultValue));
									RuntimeValue_c rtV = marshallContentIn(blockedFrame.getModelRoot(), null,
											Vm_c.getRawResult(), dt, returnArrayDepth, ci);
									rtV.relateAcrossR3305To(localVsf);
									rtV.relateAcrossR3310To(localStackFrame);
								}
								if (blockedFrame != null) {
									Stack_c invokingStack = Stack_c.getOneI_STACKOnR2943(blockedFrame);
									localStackFrame.setReadyforinterrupt(false);
									invokingStack.Enqueue(false, blockedFrame.getStack_frame_id(),
											localStackFrame.getStack_frame_id());
									localStackFrame.setReadyforinterrupt(true);
									waitingComp = ComponentInstance_c.getOneI_EXEOnR2930(invokingStack);
								}
							}
							if (waitingComp != null) {
								waitingComp.Notify();
	...
	private static ValueInStackFrame_c getWaitingReturnValue(StackFrame_c blockedFrame) {
		ValueInStackFrame_c[] vsfs = ValueInStackFrame_c.getManyI_VSFsOnR2951(blockedFrame);
	...
	private static Class<?> getClassForType(DataType_c dt) {
		...
		Package_c pkg = Package_c.getOneEP_PKGOnR8000(PackageableElement_c.getOnePE_PEOnR8001(dt));
		if (pkg != null && dt != null) {
			String className = pathToClassName(pkg.Getpath("") + "::" + dt.getName());
			...
			if (sys != null) {
				...
				if (cl != null) {
					...
					if (result == null) {
						DataType_c definition = DataType_c.getOneS_DTOnR18(UserDataType_c.getOneS_UDTOnR17(dt));
						if (definition != null) {
	...
	private static void marshallValueOut(StackFrame_c localStackFrame, PropertyParameter_c prop_param,
		...
		for (int i = 0; i < lvls.length; i++) {
			PropertyParameter_c pp = PropertyParameter_c.getOneC_PPOnR3017(lvls[i]);
			if (pp == prop_param) {
				parVal = RuntimeValue_c.getOneRV_RVLOnR3306(Local_c.getOneL_LCLOnR3001(lvls[i]));
				if (parVal == null) {
					java.util.UUID rvID = lvls[i].Getruntimevalue(localStackFrame.getStack_frame_id());
					parVal = (RuntimeValue_c) lvls[i].getModelRoot().getInstanceList(RuntimeValue_c.class)
							.getGlobal(rvID);
				}
				break;
			}
	...
	private static Object marshallStructuredValueOut(DataType_c dt, Object toMarshall, RuntimeValue_c parVal,
		...
		if (clazz != null) {
			...
			ValueInStructure_c[] viss = ValueInStructure_c.getManyRV_VISsOnR3301(strV);
			StructureMember_c[] members = StructureMember_c
					.getManyS_MBRsOnR44(StructuredDataType_c.getOneS_SDTOnR17(coreType));
			for (StructureMember_c member : members) {
				for (ValueInStructure_c vis : viss) {
					if (vis.getName().equals(member.getName())) {
						...
						RuntimeValue_c rVal = RuntimeValue_c.getOneRV_RVLOnR3301(vis);
						DataType_c memberDt = DataType_c.getOneS_DTOnR45(member);
						Object value = null;
						if (toMarshall != null) {
							try {
								...
								String ex = e.getLocalizedMessage();
								LOG.LogInfo(
										"Illegal argument exception while getting field value for realized data type: "
												+ ex);
							} catch (IllegalAccessException e) {
								String ex = e.getLocalizedMessage();
								LOG.LogInfo(
										"Illegal access exception while getting field value for realized data type: "
												+ ex);
							}
						}
	...
	private static Object marshallCoreValueOut(DataType_c dt, Object toMarshall, RuntimeValue_c parVal, boolean isByRef,
		...
		if (udt != null) {
			...
			if (inst != null) {
				...
				for (Method meth : methods) {
					if (meth.getName().equals("setValue") && meth.getParameterTypes().length == 1) {
						try {
							...
							String ex = e.getLocalizedMessage();
							LOG.LogInfo(
									"Illegal argument exception while invoking accessor value for realized data type: "
											+ ex);
						} catch (IllegalAccessException e) {
							String ex = e.getLocalizedMessage();
							LOG.LogInfo(
									"Illegal access exception while invoking accessor value for realized data type: "
											+ ex);
						} catch (InvocationTargetException e) {
							String ex = e.getLocalizedMessage();
							LOG.LogInfo(
									"Invocation target exception while invoking accessor value for realized data type: "
											+ ex);
						}
					}
				}
					...
					for (Field field : fields) {
						if (field.getName().equals("value")) {
							try {
								...
								String ex = e.getLocalizedMessage();
								LOG.LogInfo("Illegal argument exception while accessing field of realized data type: "
										+ ex);
							} catch (IllegalAccessException e) {
								String ex = e.getLocalizedMessage();
								LOG.LogInfo(
										"Illegal access exception while accessing field of realized data type: " + ex);
							}
							break;
						}
					}
		...
		if (!realizedTypeFound) {
			if (toMarshall == null) {
				toMarshall = convertToRawJavaType(parVal.Getvalue(), coreType, isByRef, ci);
			} else {
				Object result = convertToRawJavaType(parVal.Getvalue(), coreType, false, ci);
				if (isByRef == true) {
	...
	private static Object marshallArrayValueOut(Object toMarshall, DataType_c dt, RuntimeValue_c parVal, int arrayDepth,
		...
		for (ValueInArray_c vir : virs) {
			RuntimeValue_c content = RuntimeValue_c.getOneRV_RVLOnR3302(vir);
			Object value = null;
	...
	static Class<?> getClassForCoreTypeOf(DataType_c dt, Dimensions_c[] dims, boolean byRef) {
		...
		StructuredDataType_c sdt = StructuredDataType_c.getOneS_SDTOnR17(coreDt);
		EnumerationDataType_c edt = EnumerationDataType_c.getOneS_EDTOnR17(coreDt);
		UserDataType_c udt = UserDataType_c.getOneS_UDTOnR17(dt);
		...
		if (geis != null || cdt != null) {
			...
			Package_c pkg = Package_c.getOneEP_PKGOnR8000(PackageableElement_c.getOnePE_PEOnR8001(coreDt));
			if (pkg != null) {
