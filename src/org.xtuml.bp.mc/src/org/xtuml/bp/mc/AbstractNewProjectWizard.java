...
public abstract class AbstractNewProjectWizard extends DelegatingWizard {
    ...
    public void init(IWorkbench workbench, IStructuredSelection selection) {
        ...
        if (gdw != null) {
            mcis = gdw.getChoices();
            if (mcis.length > 1) {
                addPage(new WizardDelegateChooserPage("newxtUMLCodeBuilderChooser", "Multiple Code Builders found",
                        "Select the code builder to use with this xtUML project", "Available code builders:"));
    ...
    public boolean performFinish(IProject newProject) {
        ...
        Selection.getInstance().setSelection(new StructuredSelection(newSys));
