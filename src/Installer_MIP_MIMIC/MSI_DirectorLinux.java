...
public class MSI_Director {
    ...
    private String productVersion = "4.1.10";
                ...
                new HashMap() {
                    {
                        ...
                        put("messagePrompt", String.format("The %s installation has completed.<br><br>You can now use %s/Launcher.sh to start the application.", productName, eclipseLocation));
                    }
                });
    }
                ...
                new HashMap() {
                    {
                        ...
                        put(DirectorHelper.PROPERTY_BTN_SET_FOCUS, Integer.toString(DirectorHelper.BTN_NEXT));
                        put("messageIcon", "Question.gif");
                        put("messagePrompt", String.format("Please specify the existing Eclipse %s or later installation to use. For example: /usr/bin/eclipse<br><br>Additional non-eclipse application files will be installed elsewhere.  You will select the location to install those files later.", eclipseVersion) + 
    ...
    /**
     * Returns the path of the MIP install folder.
     * @return String
     */
    private String getBase() {
        String result = "";

        URL baseUrl = ClassLoader.getSystemResource("installData.xml.gz");
        if (baseUrl == null) {
            return new File(".").toString();
        }
        try {
            result = new URL(baseUrl, "../").getFile().replaceAll("%20", " ");
            while (result.length() > 0 && result.charAt(0) == '/') {
                result = result.substring(1);
            }
        } catch (Exception ex) {
            DirectorHelper.handleError(ex);
        }
    ...
    /**
     * Returns the path of the MIP tmp folder.
     * @return String
     */
    public String getTmpFolder() {
        String pid = System.getenv("MG_INSTALL_PID");
        String tmpDir = getBase() + "/tmp/" + (pid == null ? "" : pid);
