...
MCMC_EXE="com.mentor.nucleus.bp.mc.c.binary_4.1.12/mc3020/bin/mcmc.exe"
SEQUENCE_CREATOR="com.mentor.nucleus.bp.sequencecapture_4.1.12.jar"
SERVER="tucson.wv"
...
INSTALLER_DATA_DIR="V:/"
BIN_DIR="C:/MIMIC/rlstools/bin"
WKSP_LOC="C:/MIMIC/mimic_ws_bp_e3.7"
BUNDLE_NAME="${PRODUCT_NAME}"
PLATFORM="ixn"
DIRECTOR="${PRODUCT_NAME}_e3.7"
...
echo "INFO: Removing unneeded Linux plugin fragment."
cd "${INSTALLER_DATA_DIR}/${PRODUCT_NAME}/eclipse/plugins/"
rm -rf ${LINUX_FRAGMENT}*
echo "INFO: Done."
...
size=$(du -k ${BUNDLE_NAME}_${PRODUCT_VER}_${DATESTAMP}.exe | sed 's/\([0-9]*\)\(.*\)/\1/')
...
ssh ${SERVER} "(cd '${REMOTE_RELEASE_DIR}'; chmod 755 ${BUNDLE_NAME}_${PRODUCT_VER}_*.exe)"
echo "INFO: Done."

echo "INFO: Installer creation complete.  Goodbye."
