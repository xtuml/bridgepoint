...
public class GenerateFunctionsFromList implements IActionDelegate {
    ...
    class ElementData {
        ...
        public void create() {
            switch (this.type) {
            case FUNCTION:
                this.createdElement = Function_c.getOneS_SYNCOnR8001(PackageableElement_c.getManyPE_PEsOnR8000(GenerateFunctionsFromList.this.parentPackage), new ClassQueryInterface_c() {
                    @Override
                    public boolean evaluate( Object candidate ) {
                        return ((Function_c)candidate).getName().equals( ElementData.this.name );
                    }
                });
                if ( null == this.createdElement ) {
                ...
                DataType_c currentReturnType = DataType_c.getOneS_DTOnR25( ((Function_c)this.createdElement) );
                if ( null != currentReturnType ) {
                    ((Function_c) this.createdElement).unrelateAcrossR25From(currentReturnType);
                }
                ((Function_c) this.createdElement).relateAcrossR25To(returnType);
                createChildren(this.children);
                break;
            case PARAMETER:
                this.createdElement = FunctionParameter_c.getOneS_SPARMOnR24(this.parentFunction, new ClassQueryInterface_c() {
                    @Override
                    public boolean evaluate( Object candidate ) {
                        return ((FunctionParameter_c)candidate).getName().equals( ElementData.this.name );
                    }
                });
                if ( null == this.createdElement ) {
                ...
                DataType_c currentParamType = DataType_c.getOneS_DTOnR26( ((FunctionParameter_c)this.createdElement) );
                if ( null != currentParamType ) {
                    ((FunctionParameter_c) this.createdElement).unrelateAcrossR26From(currentParamType);
                }
