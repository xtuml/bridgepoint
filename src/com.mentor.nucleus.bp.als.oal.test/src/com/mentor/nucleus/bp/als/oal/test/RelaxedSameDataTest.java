//========================================================================
//
//File:      $RCSfile: RelaxedSameDataTest.java,v $
...
public class RelaxedSameDataTest extends BaseTest {
  public RelaxedSameDataTest() {
	  super("Models", "Relaxed Same Data");
  }
  ...
  protected void setUp() throws Exception {
	  super.setUp();
      ensureAvailableAndLoaded("Models", "Relaxed Same Data", false, false);
	  populateStateActionInstances();
	  OalParserTest.modelRoot = getModelRootOfTestDomain();
  }

  // The tests themselves
  public void testNoTransitionsNoParameterAccess() {
	String x = OalParserTest.parseAction(NO_PARAMETER_ACCESS,
			   OalParserTest.ACTIVITY_TYPE_STATE, NO_TRANSITIONS); //$NON-NLS-1$
	assertTrue("Unexpected error encountered: " + x, x.length() == 0);
  }
  public void testNoTransitionsNonExistentParameterAccess() {
	String x = OalParserTest.parseAction(NON_EXISTENT_PARAMETER_ACCESS,
               OalParserTest.ACTIVITY_TYPE_STATE, NO_TRANSITIONS); //$NON-NLS-1$
  ...
  public void testNoEventParametersNoParameterAccess() {
    String x = OalParserTest.parseAction(NO_PARAMETER_ACCESS,
          OalParserTest.ACTIVITY_TYPE_STATE, NO_EVENT_PARAMETERS); //$NON-NLS-1$
    assertTrue("Unexpected error encountered: " + x, x.length() == 0);
  }
  ...
  public void testNonMatchingEventParametersWithSubsetNonSubsetParameterAccess() {
    String x = OalParserTest.parseAction(NON_SUBSET_PARAMETER_ACCESS,
                                       OalParserTest.ACTIVITY_TYPE_STATE,
                       NON_MATCHING_EVENT_PARAMETERS_WITH_SUBSET); //$NON-NLS-1$
    String [] lines = x.split("\\n");
    assertEquals(3, lines.length);
  ...
  private void populateStateActionInstances() {
	StateMachine_c sm = StateMachine_c.
	                             StateMachineInstance(getModelRootOfTestDomain());
	...
	for ( int i = 0; i < i_acts.length; ++i )
	{
      ...
      OalParserTest.m_testAction[st.getNumb() - 1] = i_acts[i]; 
	}
  }
  /* (non-Javadoc)
