...
//=====================================================================
//
//File:      $RCSfile: OalParserTest.java,v $
...
public class OalParserTest extends BaseTest {
	...
	public OalParserTest(String arg0) {
	...
	protected void setUp() throws Exception {
	    ...
	    // Set up a System instance, so that the
		// Domain thinks it is a model root (UUID below
		// must match system id in Domain instance of test model.
		// This done here so that batch relate can find it.
		SystemModel_c sys = new SystemModel_c(modelRoot);
		sys.setSys_id(UUID.fromString("00000000-0000-0000-0000-000000000004"));
		Ooaofooa.getDefaultInstance().getInstanceList(SystemModel_c.class).add(sys);
		Ooaofooa.getDefaultInstance().getInstanceList(SystemModel_c.class).put(sys.getSys_id().toString(), sys);
		if (d2 == null) {
			String modelName = "testOAL1";
            PersistableModelComponent domainComponent = ensureAvailableAndLoaded(modelName, false);
            modelRoot = (Ooaofooa)domainComponent.getRootModelElement().getModelRoot();
			
//			ImportModel impMod = new ImportModel(workspace_path + Ooaofooa.MODELS_DIRNAME+ "/" +"testOAL1." + Ooaofooa.MODELS_EXT, modelRoot, null, true, true, false);//$NON-NLS-1$
//            int i = impMod.countAndValidateInsertStatements();
//            assertTrue ( i > 0 );
//			impMod.run(new NullProgressMonitor());
	        modelRoot.setLoadPathForTests(Ooaofooa.MODELS_DIRNAME + "/" + "testOAL1" + "." + Ooaofooa.MODELS_EXT); //$NON-NLS-1$
		}
		else {
          SystemModel_c testSys = SystemModel_c.getOneS_SYSOnR28(d2);
          if (testSys == null) {
	...
	public void testBridgeWithMissingReturnStmt(){
		String act = "";
		String x = parseAction(act, ACTIVITY_TYPE_BRG, missingReturnValue);
		assertEquals("", x);//$NON-NLS-1$
		
	}
	public void testFunctionWithMissingReturnStmt(){
		String act = "";
		String x = parseAction(act, ACTIVITY_TYPE_FUNC, missingReturnValue);
		assertEquals(":1:1-1: Return value required by function\n", x);//$NON-NLS-1$
		
	}
	public void testOperationWithMissingReturnStmt(){
		String act = "";
		String x = parseAction(act, ACTIVITY_TYPE_CB_OP, missingReturnValue);
		assertEquals(":1:1-1: Return value required by class operation\n", x);//$NON-NLS-1$
	...
	static public String parseAction(String stmts, int funcType, int funcNum)
	{
		...
		switch (funcType) {
			case ACTIVITY_TYPE_FUNC :
				parser.m_oal_context = new Oal_validate(DomainUtil.getDomain(m_testFunc[funcNum]));
			...
			case ACTIVITY_TYPE_BRG :
				parser.m_oal_context = new Oal_validate(DomainUtil.getDomain(m_testBrg[funcNum]));
			...
			case ACTIVITY_TYPE_CB_OP :
				parser.m_oal_context = new Oal_validate(DomainUtil.getDomain(m_testCBTfr[funcNum]));
			...
			case ACTIVITY_TYPE_IB_OP :
				parser.m_oal_context = new Oal_validate(DomainUtil.getDomain(m_testIBTfr[funcNum]));
			...
			case ACTIVITY_TYPE_MDA :
				parser.m_oal_context = new Oal_validate(DomainUtil.getDomain(m_testMDA[funcNum]));
			...
			case ACTIVITY_TYPE_STATE :
				parser.m_oal_context = new Oal_validate(DomainUtil.getDomain(m_testAction[funcNum]));
	...
	static public final int missingReturnValue = 21;
	...
	private void populateStateActionInstances() {
        ...
        for ( int i = 0; i < t_acts.length; ++i )
        {
	      ...
	      TransitionActionHome_c tah = TransitionActionHome_c.getOneSM_TAHOnR513(ah);
	      Transition_c tran = Transition_c.getOneSM_TXNOnR530(tah);
		  StateMachineState_c dest = StateMachineState_c
					.getOneSM_STATEOnR503(StateEventMatrixEntry_c
							.getOneSM_SEMEOnR504(NewStateTransition_c
									.getOneSM_NSTXNOnR507(tran)));
	      if (dest.getName().contains("PolyTwo"))//$NON-NLS-1$
	        m_testAction[TRANS_ISM_POLYTWO] = t_acts[i];
	      else if ( dest.getName().contains("PolyOrphaned") )//$NON-NLS-1$
