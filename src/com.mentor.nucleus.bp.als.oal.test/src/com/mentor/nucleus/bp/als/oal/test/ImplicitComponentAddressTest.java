...
public class ImplicitComponentAddressTest extends BaseTest {
	...
	private String parseAction(String stmts, int opType, String actionName) {
		...
		if (opType == Oalconstants_c.OPERATION_TYPE) {
			...
			actID = handleBridgeTest(dom, stmts, names);
		}
		else if (opType == Oalconstants_c.FUNCTION_TYPE) {
			actID = handleFunctionTest(dom, stmts, names);
		}
    ...
    UUID handleBridgeTest(Domain_c dom, String stmts, String [] names) {
    	ExternalEntity_c eeUT = null;
    	ExternalEntity_c[] ees = ExternalEntity_c.getManyS_EEsOnR8(dom);
    ...
    UUID handleFunctionTest(Domain_c dom, String stmts, String [] names) {
    	Function_c fnUT = null;
    	Function_c [] fns = Function_c.getManyS_SYNCsOnR23(dom);
