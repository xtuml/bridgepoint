...
import org.xtuml.bp.core.BodyInElement_c;
...
public class ParserAllActivityModifier implements IAllActivityModifier {
	...
	public ParserAllActivityModifier(SystemModel_c parent, IProgressMonitor monitor) {
		this((NonRootModelElement) parent, monitor);
		parseAll = true;
	}
	...
	public ParserAllActivityModifier(NonRootModelElement parent, IProgressMonitor monitor) {
		...
		} else if (parent instanceof SystemModel_c) {
			m_sysMod = (SystemModel_c) parent;
		}
		m_pm = monitor;
	}
	...
	public void parseAction(Object modelElement) {
		...
		if (modelElement instanceof Function_c) {
			...
			IllegalArgumentException ia = new IllegalArgumentException(
					"AllActivityModifier::parseAction encountered an unexpeced type: "
							+ modelElement.getClass().toString());
	...
	public static int accessSuc_Pars(boolean writeValue, int newValue, Object o_input) {
		if (o_input instanceof Function_c) {
			...
			DerivedBaseAttribute_c dba = DerivedBaseAttribute_c
					.getOneO_DBATTROnR107(BaseAttribute_c.getOneO_BATTROnR106((Attribute_c) o_input));
		...
		if (!writeValue) {
			IllegalArgumentException ia = new IllegalArgumentException(
					"Unsupported Action Home:" + o_input.getClass().toString());
	...
	private enum ActivityKind {
		BRIDGE, FUNCTION, MDA, OPERATION, PROVIDED_OP, REQUIRED_OP, PROVIDED_SIGNAL, REQUIRED_SIGNAL, ISM_STATE, CSM_STATE, ISM_TXN, CSM_TXN
	};

	private void addActivitiesToList(ArrayList<NonRootModelElement> act_set, PackageableElement_c pkgElem,
			ActivityKind kind, Package_c rootPkg) {
		addActivitiesToList(act_set, pkgElem, kind, rootPkg, false);
	}

	private void addActivitiesToList(ArrayList<NonRootModelElement> act_set, PackageableElement_c pkgElem,
			ActivityKind kind, Package_c rootPkg, boolean onlySharedFragments) {
		...
		if (kind == ActivityKind.BRIDGE) {
			...
			nrme_set = Attribute_c.getManyO_ATTRsOnR106(BaseAttribute_c.getManyO_BATTRsOnR107(
					DerivedBaseAttribute_c.getManyO_DBATTRsOnR107(BaseAttribute_c.getManyO_BATTRsOnR106(
							Attribute_c.getManyO_ATTRsOnR102(ModelClass_c.getManyO_OBJsOnR8001(contents))))));
		} else if (kind == ActivityKind.OPERATION) {
			nrme_set = Operation_c.getManyO_TFRsOnR115(ModelClass_c.getManyO_OBJsOnR8001(contents));
		} else if (kind == ActivityKind.PROVIDED_OP) {
			nrme_set = ProvidedOperation_c.getManySPR_POsOnR4503(
					ProvidedExecutableProperty_c.getManySPR_PEPsOnR4501(Provision_c.getManyC_PsOnR4009(
							InterfaceReference_c.getManyC_IRsOnR4016(Port_c.getManyC_POsOnR4010(comp)))));
		} else if (kind == ActivityKind.REQUIRED_OP) {
			nrme_set = RequiredOperation_c.getManySPR_ROsOnR4502(
			...
			nrme_set = ProvidedSignal_c.getManySPR_PSsOnR4503(
					ProvidedExecutableProperty_c.getManySPR_PEPsOnR4501(Provision_c.getManyC_PsOnR4009(
							InterfaceReference_c.getManyC_IRsOnR4016(Port_c.getManyC_POsOnR4010(comp)))));
		} else if (kind == ActivityKind.REQUIRED_SIGNAL) {
			nrme_set = RequiredSignal_c.getManySPR_RSsOnR4502(
					RequiredExecutableProperty_c.getManySPR_REPsOnR4500(Requirement_c.getManyC_RsOnR4009(
							InterfaceReference_c.getManyC_IRsOnR4016(Port_c.getManyC_POsOnR4010(comp)))));
		} else if (kind == ActivityKind.ISM_STATE) {
			nrme_set = StateMachineState_c.getManySM_STATEsOnR501(StateMachine_c.getManySM_SMsOnR517(
					InstanceStateMachine_c.getManySM_ISMsOnR518(ModelClass_c.getManyO_OBJsOnR8001(contents))));
		} else if (kind == ActivityKind.CSM_STATE) {
			nrme_set = StateMachineState_c.getManySM_STATEsOnR501(StateMachine_c.getManySM_SMsOnR517(
					ClassStateMachine_c.getManySM_ASMsOnR519(ModelClass_c.getManyO_OBJsOnR8001(contents))));
		} else if (kind == ActivityKind.ISM_TXN) {
			nrme_set = Transition_c.getManySM_TXNsOnR505(StateMachine_c.getManySM_SMsOnR517(
					InstanceStateMachine_c.getManySM_ISMsOnR518(ModelClass_c.getManyO_OBJsOnR8001(contents))));
			...
			Throwable e = new Throwable();
			e.fillInStackTrace();
			CorePlugin.logError("Unrecognized activity kind in addActivitiesToList", e);
		}
		...
		if (!onlySharedFragments) {
			...
			for (int i = 0; i < pkg_set.length; ++i) {
				addActivitiesToList(act_set, PackageableElement_c.getOnePE_PEOnR8001(pkg_set[i]), kind, null);
			}
			for (int i = 0; i < comp_set.length; ++i) {
				addActivitiesToList(act_set, PackageableElement_c.getOnePE_PEOnR8001(comp_set[i]), kind, null);
			}
		}
	}
	...
	private void getAllInterfaceSignals() {
		...
		if (parseAll && m_pkgElem != null) {
			...
			addActivitiesToList(act_set, m_pkgElem, ActivityKind.REQUIRED_SIGNAL, null);
			m_required_signals = act_set.toArray(new RequiredSignal_c[act_set.size()]);
			...
			act_set.clear();
			addActivitiesToList(act_set, m_pkgElem, ActivityKind.PROVIDED_SIGNAL, null);
			...
			ArrayList<NonRootModelElement> act_set = new ArrayList<NonRootModelElement>();
			addActivitiesToList(act_set, null, ActivityKind.REQUIRED_SIGNAL, m_pkg);
			m_required_signals = act_set.toArray(new RequiredSignal_c[act_set.size()]);
			assert (m_required_signals == rs_set.toArray(new RequiredSignal_c[rs_set.size()]));
			ArrayList<ProvidedSignal_c> pv_set = new ArrayList<ProvidedSignal_c>();
			addProvidedSignalsToList(pv_set, m_pkg);
			act_set.clear();
			addActivitiesToList(act_set, null, ActivityKind.PROVIDED_SIGNAL, m_pkg);
			m_provided_signals = act_set.toArray(new ProvidedSignal_c[act_set.size()]);
			assert (m_provided_signals == pv_set.toArray(new ProvidedSignal_c[pv_set.size()]));
			...
			for (int i = 0; i < packages.length; i++) {
				addRequiredSignalsToList(rs_set, packages[i]);

				addActivitiesToList(act_set, null, ActivityKind.REQUIRED_SIGNAL, packages[i]);
			}
			m_required_signals = act_set.toArray(new RequiredSignal_c[act_set.size()]);
			assert (m_required_signals == rs_set.toArray(new RequiredSignal_c[rs_set.size()]));
			ArrayList<ProvidedSignal_c> pv_set = new ArrayList<ProvidedSignal_c>();
			act_set.clear();
			for (int i = 0; i < packages.length; i++) {
				addProvidedSignalsToList(pv_set, packages[i]);
				addActivitiesToList(act_set, null, ActivityKind.PROVIDED_SIGNAL, packages[i]);
			}
			m_provided_signals = act_set.toArray(new ProvidedSignal_c[act_set.size()]);
			assert (m_provided_signals == pv_set.toArray(new ProvidedSignal_c[pv_set.size()]));
		}
	...
	private void getAllInterfaceOperations() {
		...
		if (parseAll && m_parent instanceof Component_c) {
			...
			m_required_operations = RequiredOperation_c.getManySPR_ROsOnR4502(
					RequiredExecutableProperty_c.getManySPR_REPsOnR4500(Requirement_c.getManyC_RsOnR4009(
							InterfaceReference_c.getManyC_IRsOnR4016(Port_c.getManyC_POsOnR4010(component)))));
			m_provided_operations = ProvidedOperation_c.getManySPR_POsOnR4503(
					ProvidedExecutableProperty_c.getManySPR_PEPsOnR4501(Provision_c.getManyC_PsOnR4009(
							InterfaceReference_c.getManyC_IRsOnR4016(Port_c.getManyC_POsOnR4010(component)))));
			...
			m_required_operations = act_set.toArray(new RequiredOperation_c[act_set.size()]);
			assert (m_required_operations == ro_set.toArray(new RequiredOperation_c[ro_set.size()]));
			ArrayList<ProvidedOperation_c> po_set = new ArrayList<ProvidedOperation_c>();
			...
			for (int i = 0; i < packages.length; i++) {
				addRequiredOperationsToList(ro_set, packages[i]);
				addActivitiesToList(act_set, null, ActivityKind.REQUIRED_OP, packages[i]);
			}
			m_required_operations = act_set.toArray(new RequiredOperation_c[act_set.size()]);
			assert (m_required_operations == ro_set.toArray(new RequiredOperation_c[ro_set.size()]));
			ArrayList<ProvidedOperation_c> po_set = new ArrayList<ProvidedOperation_c>();
			act_set.clear();
			for (int i = 0; i < packages.length; i++) {
				addProvidedOperationsToList(po_set, packages[i]);
				addActivitiesToList(act_set, null, ActivityKind.PROVIDED_OP, packages[i]);
			}
			m_provided_operations = act_set.toArray(new ProvidedOperation_c[act_set.size()]);
			assert (m_provided_operations == po_set.toArray(new ProvidedOperation_c[po_set.size()]));
		}
	...
	private void addRequiredSignalsToList(ArrayList<RequiredSignal_c> rs_set, PackageableElement_c pkgElem) {
		...
		if (pkg != null) {
			r_set = RequiredSignal_c
					.getManySPR_RSsOnR4502(RequiredExecutableProperty_c.getManySPR_REPsOnR4500(Requirement_c
							.getManyC_RsOnR4009(InterfaceReference_c.getManyC_IRsOnR4016(Port_c.getManyC_POsOnR4010(
									Component_c.getManyC_CsOnR8001(PackageableElement_c.getManyPE_PEsOnR8000(pkg)))))));
			...
			Component_c comp = Component_c.getOneC_COnR8001(pkgElem);
			if (comp != null) {
				r_set = RequiredSignal_c.getManySPR_RSsOnR4502(
						RequiredExecutableProperty_c.getManySPR_REPsOnR4500(Requirement_c.getManyC_RsOnR4009(
								InterfaceReference_c.getManyC_IRsOnR4016(Port_c.getManyC_POsOnR4010(comp)))));
				pkg_set = Package_c.getManyEP_PKGsOnR8001(PackageableElement_c.getManyPE_PEsOnR8003(comp));
				comp_set = Component_c.getManyC_CsOnR8001(PackageableElement_c.getManyPE_PEsOnR8003(comp));
			}
		}
	...
	private void addProvidedSignalsToList(ArrayList<ProvidedSignal_c> pv_set, PackageableElement_c pkgElem) {
		...
		Package_c pkg = Package_c.getOneEP_PKGOnR8001(pkgElem);
		if (pkg != null) {
			p_set = ProvidedSignal_c.getManySPR_PSsOnR4503(ProvidedExecutableProperty_c.getManySPR_PEPsOnR4501(
					Provision_c.getManyC_PsOnR4009(InterfaceReference_c.getManyC_IRsOnR4016(Port_c.getManyC_POsOnR4010(
							Component_c.getManyC_CsOnR8001(PackageableElement_c.getManyPE_PEsOnR8000(pkg)))))));
			pkg_set = Package_c.getManyEP_PKGsOnR8001(PackageableElement_c.getManyPE_PEsOnR8000(pkg));
			comp_set = Component_c.getManyC_CsOnR8001(PackageableElement_c.getManyPE_PEsOnR8000(pkg));
		} else {
			Component_c comp = Component_c.getOneC_COnR8001(pkgElem);
			if (comp != null) {
				p_set = ProvidedSignal_c.getManySPR_PSsOnR4503(
						ProvidedExecutableProperty_c.getManySPR_PEPsOnR4501(Provision_c.getManyC_PsOnR4009(
								InterfaceReference_c.getManyC_IRsOnR4016(Port_c.getManyC_POsOnR4010(comp)))));
				pkg_set = Package_c.getManyEP_PKGsOnR8001(PackageableElement_c.getManyPE_PEsOnR8003(comp));
				comp_set = Component_c.getManyC_CsOnR8001(PackageableElement_c.getManyPE_PEsOnR8003(comp));
	...
	private void addRequiredOperationsToList(ArrayList<RequiredOperation_c> rs_set, PackageableElement_c pkgElem) {
		...
		if (pkg != null) {
			r_set = RequiredOperation_c
					.getManySPR_ROsOnR4502(RequiredExecutableProperty_c.getManySPR_REPsOnR4500(Requirement_c
							.getManyC_RsOnR4009(InterfaceReference_c.getManyC_IRsOnR4016(Port_c.getManyC_POsOnR4010(
									Component_c.getManyC_CsOnR8001(PackageableElement_c.getManyPE_PEsOnR8000(pkg)))))));
			pkg_set = Package_c.getManyEP_PKGsOnR8001(PackageableElement_c.getManyPE_PEsOnR8000(pkg));
			comp_set = Component_c.getManyC_CsOnR8001(PackageableElement_c.getManyPE_PEsOnR8000(pkg));
			...
			Component_c comp = Component_c.getOneC_COnR8001(pkgElem);
			if (comp != null) {
				r_set = RequiredOperation_c.getManySPR_ROsOnR4502(
						RequiredExecutableProperty_c.getManySPR_REPsOnR4500(Requirement_c.getManyC_RsOnR4009(
								InterfaceReference_c.getManyC_IRsOnR4016(Port_c.getManyC_POsOnR4010(comp)))));
				pkg_set = Package_c.getManyEP_PKGsOnR8001(PackageableElement_c.getManyPE_PEsOnR8003(comp));
				comp_set = Component_c.getManyC_CsOnR8001(PackageableElement_c.getManyPE_PEsOnR8003(comp));
	...
	private void addProvidedOperationsToList(ArrayList<ProvidedOperation_c> pv_set, PackageableElement_c pkgElem) {
		...
		Package_c pkg = Package_c.getOneEP_PKGOnR8001(pkgElem);
		if (pkg != null) {
			p_set = ProvidedOperation_c.getManySPR_POsOnR4503(ProvidedExecutableProperty_c.getManySPR_PEPsOnR4501(
					Provision_c.getManyC_PsOnR4009(InterfaceReference_c.getManyC_IRsOnR4016(Port_c.getManyC_POsOnR4010(
							Component_c.getManyC_CsOnR8001(PackageableElement_c.getManyPE_PEsOnR8000(pkg)))))));
			pkg_set = Package_c.getManyEP_PKGsOnR8001(PackageableElement_c.getManyPE_PEsOnR8000(pkg));
			comp_set = Component_c.getManyC_CsOnR8001(PackageableElement_c.getManyPE_PEsOnR8000(pkg));
			...
			Component_c comp = Component_c.getOneC_COnR8001(pkgElem);
			if (comp != null) {
				p_set = ProvidedOperation_c.getManySPR_POsOnR4503(
						ProvidedExecutableProperty_c.getManySPR_PEPsOnR4501(Provision_c.getManyC_PsOnR4009(
								InterfaceReference_c.getManyC_IRsOnR4016(Port_c.getManyC_POsOnR4010(comp)))));
				pkg_set = Package_c.getManyEP_PKGsOnR8001(PackageableElement_c.getManyPE_PEsOnR8003(comp));
				comp_set = Component_c.getManyC_CsOnR8001(PackageableElement_c.getManyPE_PEsOnR8003(comp));
	...
	private int countAllActivities() {
		...
		return ((m_func_set != null) ? m_func_set.length : 0) + ((m_bridge_set != null) ? m_bridge_set.length : 0)
				+ ((m_op_set != null) ? m_op_set.length : 0) + ((m_mda_set != null) ? m_mda_set.length : 0)
	...
	private void getAllFunctions(boolean onlySharedFragments) {
		...
		if (m_func_set != null)
			m_func_set = null; // clear any existing entries
		if (parseAll && m_pkgElem != null) {
			// 
			...
			ArrayList<Function_c> function_set = new ArrayList<Function_c>();
			for (int i = 0; i < packages.length; i++) {
				addFunctionsToList(function_set, packages[i], onlySharedFragments);
			}
			ArrayList<NonRootModelElement> act_set = new ArrayList<NonRootModelElement>();
			for (int i = 0; i < packages.length; i++) {
				addActivitiesToList(act_set, null, ActivityKind.FUNCTION, packages[i], onlySharedFragments);
			}
			m_func_set = act_set.toArray(new Function_c[act_set.size()]);
			assert (m_func_set == function_set.toArray(new Function_c[function_set.size()]));
		} else {
			m_func_set = (Function_c[]) PersistenceManager.getHierarchyMetaData().getActivityModelElements(m_parent,
					Function_c.class);
		}
	...
	private void addFunctionsToList(ArrayList<Function_c> function_set, PackageableElement_c pkgElem) {
		...
		if (pkg != null) {
			...
			Component_c comp = Component_c.getOneC_COnR8001(pkgElem);
			if (comp != null) {
				f_set = Function_c.getManyS_SYNCsOnR8001(PackageableElement_c.getManyPE_PEsOnR8003(comp));
				pkg_set = Package_c.getManyEP_PKGsOnR8001(PackageableElement_c.getManyPE_PEsOnR8003(comp));
				comp_set = Component_c.getManyC_CsOnR8001(PackageableElement_c.getManyPE_PEsOnR8003(comp));
			}
		}
	...
	private void getAllBridges(boolean onlySharedFragments) {
		...
		if (m_bridge_set != null)
			m_bridge_set = null; // clear any existing entries
		if (parseAll && m_pkgElem != null) {
			// 
			...
			ArrayList<NonRootModelElement> act_set = new ArrayList<NonRootModelElement>();
			for (int i = 0; i < packages.length; i++) {
				addBridgesToList(bridge_set, packages[i], onlySharedFragments);
				addActivitiesToList(act_set, null, ActivityKind.BRIDGE, packages[i], onlySharedFragments);
			}
			m_bridge_set = act_set.toArray(new Bridge_c[act_set.size()]);
			assert (m_bridge_set == bridge_set.toArray(new Bridge_c[bridge_set.size()]));
		} else {
			m_bridge_set = (Bridge_c[]) PersistenceManager.getHierarchyMetaData().getActivityModelElements(m_parent,
					Bridge_c.class);
		}
	...
	private void addBridgesToList(ArrayList<Bridge_c> bridge_set, PackageableElement_c pkgElem) {
		...
		if (pkg != null) {
			...
			if (comp != null) {
				b_set = Bridge_c.getManyS_BRGsOnR19(
						ExternalEntity_c.getManyS_EEsOnR8001(PackageableElement_c.getManyPE_PEsOnR8003(comp)));
				pkg_set = Package_c.getManyEP_PKGsOnR8001(PackageableElement_c.getManyPE_PEsOnR8003(comp));
				comp_set = Component_c.getManyC_CsOnR8001(PackageableElement_c.getManyPE_PEsOnR8003(comp));
			}
		}
	...
	private void getAllOperations() {
		if (m_op_set != null)
			m_op_set = null; // clear any existing entries
		if (parseAll && m_pkgElem != null) {
			...
			Package_c[] packages = Package_c.getManyEP_PKGsOnR1401(m_sysMod);
			for (int i = 0; i < packages.length; i++) {
				addOperationsToList(op_set, packages[i]);
			}
			ArrayList<NonRootModelElement> act_set = new ArrayList<NonRootModelElement>();
			for (int i = 0; i < packages.length; i++) {
				addActivitiesToList(act_set, null, ActivityKind.OPERATION, packages[i]);
			}
			m_op_set = act_set.toArray(new Operation_c[act_set.size()]);
			assert (m_op_set == op_set.toArray(new Operation_c[op_set.size()]));
		} else {
			m_op_set = (Operation_c[]) PersistenceManager.getHierarchyMetaData().getActivityModelElements(m_parent,
					Operation_c.class);
		}
	}
	private void addOperationsToList(ArrayList<Operation_c> op_set, PackageableElement_c pkgElem) {
		...
		if (pkg != null) {
			...
			if (comp != null) {
				o_set = Operation_c.getManyO_TFRsOnR115(
						ModelClass_c.getManyO_OBJsOnR8001(PackageableElement_c.getManyPE_PEsOnR8003(comp)));
				pkg_set = Package_c.getManyEP_PKGsOnR8001(PackageableElement_c.getManyPE_PEsOnR8003(comp));
				comp_set = Component_c.getManyC_CsOnR8001(PackageableElement_c.getManyPE_PEsOnR8003(comp));
			}
		}
	...
	private void getAllMDAttrs() {
		if (m_mda_set != null)
			m_mda_set = null; // clear any existing entries
		if (parseAll && m_pkgElem != null) {
			...
			Package_c[] packages = Package_c.getManyEP_PKGsOnR1401(m_sysMod);
			for (int i = 0; i < packages.length; i++) {
				addMDAsToList(attr_set, packages[i]);
			}
			ArrayList<NonRootModelElement> act_set = new ArrayList<NonRootModelElement>();
			for (int i = 0; i < packages.length; i++) {
				addActivitiesToList(act_set, null, ActivityKind.MDA, packages[i]);
			}
			m_mda_set = act_set.toArray(new Attribute_c[act_set.size()]);
			assert (m_mda_set == attr_set.toArray(new Attribute_c[attr_set.size()]));
		} else {
			DerivedBaseAttribute_c[] attrs = (DerivedBaseAttribute_c[]) PersistenceManager.getHierarchyMetaData()
					.getActivityModelElements(m_parent, DerivedBaseAttribute_c.class);
			m_mda_set = Attribute_c.getManyO_ATTRsOnR106(BaseAttribute_c.getManyO_BATTRsOnR107(attrs));
		}
	}
	...
	private void getAllStateActivities() {
		if (m_ism_state_set != null)
			m_ism_state_set = null; // clear any existing entries
		if (parseAll && m_pkgElem != null) {
			...
			Package_c[] packages = Package_c.getManyEP_PKGsOnR1401(m_sysMod);
			for (int i = 0; i < packages.length; i++) {
				addStatesToList(state_set, packages[i], SMKind.instanceBased);
			}
			ArrayList<NonRootModelElement> act_set = new ArrayList<NonRootModelElement>();
			for (int i = 0; i < packages.length; i++) {
				addActivitiesToList(act_set, null, ActivityKind.ISM_STATE, packages[i]);
			}
			m_ism_state_set = act_set.toArray(new StateMachineState_c[act_set.size()]);
			assert (m_ism_state_set == state_set.toArray(new StateMachineState_c[state_set.size()]));
		} else {
			m_ism_state_set = (StateMachineState_c[]) PersistenceManager.getHierarchyMetaData()
					.getActivityModelElements(m_parent, StateMachineState_c.class);
		}

		if (m_csm_state_set != null)
			m_csm_state_set = null; // clear any existing entries
		if (parseAll && m_pkgElem != null) {
			...
			Package_c[] packages = Package_c.getManyEP_PKGsOnR1401(m_sysMod);
			for (int i = 0; i < packages.length; i++) {
				addStatesToList(state_set, packages[i], SMKind.classBased);
			}
			ArrayList<NonRootModelElement> act_set = new ArrayList<NonRootModelElement>();
			for (int i = 0; i < packages.length; i++) {
				addActivitiesToList(act_set, null, ActivityKind.CSM_STATE, packages[i]);
			}
			m_csm_state_set = act_set.toArray(new StateMachineState_c[act_set.size()]);
			assert (m_csm_state_set == state_set.toArray(new StateMachineState_c[state_set.size()]));
		} else {
			m_csm_state_set = (StateMachineState_c[]) PersistenceManager.getHierarchyMetaData()
					.getActivityModelElements(m_parent, StateMachineState_c.class);
		}
	}
	private enum SMKind {
		instanceBased, classBased
	};
	private void addStatesToList(ArrayList<StateMachineState_c> state_set, PackageableElement_c pkgElem, SMKind kind) {
		...
		if (pkg != null) {
			if (kind == SMKind.instanceBased) {
				...
				st_set = StateMachineState_c.getManySM_STATEsOnR501(
						StateMachine_c.getManySM_SMsOnR517(ClassStateMachine_c.getManySM_ASMsOnR519(
								ModelClass_c.getManyO_OBJsOnR8001(PackageableElement_c.getManyPE_PEsOnR8000(pkg)))));
			}
			pkg_set = Package_c.getManyEP_PKGsOnR8001(PackageableElement_c.getManyPE_PEsOnR8000(pkg));
			comp_set = Component_c.getManyC_CsOnR8001(PackageableElement_c.getManyPE_PEsOnR8000(pkg));
				...
				if (kind == SMKind.instanceBased) {
					st_set = StateMachineState_c.getManySM_STATEsOnR501(
							StateMachine_c.getManySM_SMsOnR517(InstanceStateMachine_c.getManySM_ISMsOnR518(ModelClass_c
									.getManyO_OBJsOnR8001(PackageableElement_c.getManyPE_PEsOnR8003(comp)))));
				} else {
					st_set = StateMachineState_c.getManySM_STATEsOnR501(
	...
	private void getAllTransitionActivities() {
		if (m_ism_transitions != null)
			m_ism_transitions = null;
		if (parseAll && m_pkgElem != null) {
			...
			Package_c[] packages = Package_c.getManyEP_PKGsOnR1401(m_sysMod);
			for (int i = 0; i < packages.length; i++) {
				addTransitionsToList(tran_set, packages[i], SMKind.instanceBased);
			}
			ArrayList<NonRootModelElement> act_set = new ArrayList<NonRootModelElement>();
			for (int i = 0; i < packages.length; i++) {
				addActivitiesToList(act_set, null, ActivityKind.ISM_TXN, packages[i]);
			}
			m_ism_transitions = act_set.toArray(new Transition_c[act_set.size()]);
			assert (m_ism_transitions == tran_set.toArray(new Transition_c[tran_set.size()]));
		} else {
			m_ism_transitions = (Transition_c[]) PersistenceManager.getHierarchyMetaData()
					.getActivityModelElements(m_parent, Transition_c.class);
		}
		if (m_csm_transitions != null)
			m_csm_transitions = null;
		if (parseAll && m_pkgElem != null) {
			...
			Package_c[] packages = Package_c.getManyEP_PKGsOnR1401(m_sysMod);
			for (int i = 0; i < packages.length; i++) {
				addTransitionsToList(tran_set, packages[i], SMKind.classBased);
			}
			ArrayList<NonRootModelElement> act_set = new ArrayList<NonRootModelElement>();
			for (int i = 0; i < packages.length; i++) {
				addActivitiesToList(act_set, null, ActivityKind.CSM_TXN, packages[i]);
			}
			m_csm_transitions = act_set.toArray(new Transition_c[act_set.size()]);
			assert (m_csm_transitions == tran_set.toArray(new Transition_c[tran_set.size()]));
		} else {
			m_csm_transitions = (Transition_c[]) PersistenceManager.getHierarchyMetaData()
					.getActivityModelElements(m_parent, Transition_c.class);
		}
	}
	...
	public static void initializeBodies(final ModelRoot modelRoot, final NonRootModelElement nrme) {
		...
		if (nrme != null) {
			if (nrme instanceof Component_c) {
				...
				bodies = Body_c.getManyACT_ACTsOnR694(BodyInComponent_c.getManyACT_BICsOnR694((Component_c) nrme));
				...
				bodies = Body_c.getManyACT_ACTsOnR640(BodyInElement_c.getManyACT_BIEsOnR640(pe));
				// Recursively descend the component containment hierarchy
				Component_c[] comps = Component_c
						.getManyC_CsOnR8001(PackageableElement_c.getManyPE_PEsOnR8000((Package_c) nrme));
				for (int i = 0; i < comps.length; i++) {
					initializeBodies(modelRoot, comps[i]);
				}
				// recursively descend packages
				Package_c[] pkgs = Package_c
						.getManyEP_PKGsOnR8001(PackageableElement_c.getManyPE_PEsOnR8000((Package_c) nrme));
	 ...
	 * @param nrme
	 */
	public static void initializePotentialSharedBodies(final NonRootModelElement nrme) {
		if (nrme == null) {
			return;
		}
		...
		while (pkg != null) {
			...
			FunctionBody_c[] functionBodies = FunctionBody_c
					.getManyACT_FNBsOnR698(Body_c.getManyACT_ACTsOnR640(BodyInElement_c.getManyACT_BIEsOnR640(pe)));
			...
			BridgeBody_c[] bridgeBodies = BridgeBody_c
					.getManyACT_BRBsOnR698(Body_c.getManyACT_ACTsOnR640(BodyInElement_c.getManyACT_BIEsOnR640(pe)));
...
public class ParserAllActivityModifier implements IAllActivityModifier {
	...
	private void getAllInterfaceSignals() {
		...
		if (parseAll && m_domain != null && m_parent instanceof Component_c) {
			Component_c component = ((Component_c) m_parent);
			m_required_signals = RequiredSignal_c.getManySPR_RSsOnR4502(
					RequiredExecutableProperty_c.getManySPR_REPsOnR4500(Requirement_c.getManyC_RsOnR4009(
							InterfaceReference_c.getManyC_IRsOnR4016(Port_c.getManyC_POsOnR4010(component)))));
			m_provided_signals = ProvidedSignal_c.getManySPR_PSsOnR4503(
					ProvidedExecutableProperty_c.getManySPR_PEPsOnR4501(Provision_c.getManyC_PsOnR4009(
							InterfaceReference_c.getManyC_IRsOnR4016(Port_c.getManyC_POsOnR4010(component)))));
		} else if (parseAll && m_pkgElem != null) {
			// isInGenericPackage == true
	...
	private void getAllFunctions(boolean onlySharedFragments) {
		...
		if (parseAll && m_domain != null) {
			...
			ArrayList<Function_c> function_set = new ArrayList<Function_c>();
			FunctionPackage_c[] fpk_set = FunctionPackage_c
					.getManyS_FPKsOnR30(FunctionPackageInPackage_c.getManyS_FPIPsOnR30(top));
	...
	private void loadChildFPKs(ArrayList function_set, FunctionPackage_c fpk) {
		FunctionPackage_c[] child_fpk_set = FunctionPackage_c
				.getManyS_FPKsOnR32(FunctionPackageInPackage_c.getManyS_FPIPsOnR30(fpk));
	...
	private void getAllBridges(boolean onlySharedFragments) {
		...
		if (parseAll && m_domain != null) {
			...
			ArrayList<Bridge_c> bridge_set = new ArrayList<Bridge_c>();
			ExternalEntityPackage_c[] eepk_set = ExternalEntityPackage_c
					.getManyS_EEPKsOnR33(ExternalEntityInPackage_c.getManyS_EEIPsOnR33(top));
	...
	private void loadChildEEPKs(ArrayList bridge_set, ExternalEntityPackage_c eepk) {
		ExternalEntityPackage_c[] child_eepk_set = ExternalEntityPackage_c
				.getManyS_EEPKsOnR35(EePackageInPackage_c.getManyS_EEPIPsOnR34(eepk));
	...
	private void getAllSubsystems() {
		if (m_ss_set != null)
			m_ss_set = null; // clear any existing entries
		if (parseAll && m_domain != null) {
			...
			if (m_parent instanceof Subsystem_c) {
				...
				Subsystem_c[] ss_set = Subsystem_c
						.getManyS_SSsOnR41(SubsystemInSubsystem_c.getManyS_SISsOnR41((Subsystem_c) m_parent));
				for (int i = 0; i < ss_set.length; ++i) {
					ss_result_set.add(ss_set[i]);
					loadChildSSs(ss_result_set, ss_set[i]);
				}
	...
	private void loadChildSSs(ArrayList ss_result_set, Subsystem_c ss) {
		Subsystem_c[] child_ss_set = Subsystem_c
				.getManyS_SSsOnR42(SubsystemInSubsystem_c.getManyS_SISsOnR41((Subsystem_c) ss));
	...
	private void getAllOperations() {
		...
		if (parseAll && m_ss_set != null) {
			m_op_set = Operation_c.getManyO_TFRsOnR115(ModelClass_c.getManyO_OBJsOnR2(m_ss_set));
		} else if (parseAll && m_pkgElem != null) {
			// isInGenericPackage == true
			ArrayList<Operation_c> op_set = new ArrayList<Operation_c>();
			addOperationsToList(op_set, m_pkgElem);
	...
	private void getAllMDAttrs() {
		if (m_mda_set != null)
			m_mda_set = null; // clear any existing entries
		if (parseAll && m_ss_set != null) {
			m_mda_set = Attribute_c.getManyO_ATTRsOnR106(BaseAttribute_c.getManyO_BATTRsOnR107(
					DerivedBaseAttribute_c.getManyO_DBATTRsOnR107(BaseAttribute_c.getManyO_BATTRsOnR106(
							Attribute_c.getManyO_ATTRsOnR102(ModelClass_c.getManyO_OBJsOnR2(m_ss_set))))));
		} else if (parseAll && m_pkgElem != null) {
			// isInGenericPackage == true
			ArrayList<Attribute_c> attr_set = new ArrayList<Attribute_c>();
			addMDAsToList(attr_set, m_pkgElem);
	...
	private void getAllStateActivities() {
		if (m_ism_state_set != null)
			m_ism_state_set = null; // clear any existing entries
		if (parseAll && m_ss_set != null) {
			m_ism_state_set = StateMachineState_c.getManySM_STATEsOnR501(StateMachine_c.getManySM_SMsOnR517(
					InstanceStateMachine_c.getManySM_ISMsOnR518(ModelClass_c.getManyO_OBJsOnR2(m_ss_set))));
		} else if (parseAll && m_pkgElem != null) {
			// isInGenericPackage == true
			ArrayList<StateMachineState_c> state_set = new ArrayList<StateMachineState_c>();
			addStatesToList(state_set, m_pkgElem, SMKind.instanceBased);
		...
		if (m_csm_state_set != null)
			m_csm_state_set = null; // clear any existing entries
		if (parseAll && m_ss_set != null) {
			m_csm_state_set = StateMachineState_c.getManySM_STATEsOnR501(StateMachine_c.getManySM_SMsOnR517(
					ClassStateMachine_c.getManySM_ASMsOnR519(ModelClass_c.getManyO_OBJsOnR2(m_ss_set))));
		} else if (parseAll && m_pkgElem != null) {
			// isInGenericPackage == true
			ArrayList<StateMachineState_c> state_set = new ArrayList<StateMachineState_c>();
			addStatesToList(state_set, m_pkgElem, SMKind.classBased);
	...
	private void getAllTransitionActivities() {
		if (m_ism_transitions != null)
			m_ism_transitions = null;
		if (parseAll && m_ss_set != null) {
			m_ism_transitions = Transition_c.getManySM_TXNsOnR505(StateMachine_c.getManySM_SMsOnR517(
					InstanceStateMachine_c.getManySM_ISMsOnR518(ModelClass_c.getManyO_OBJsOnR2(m_ss_set))));
		} else if (parseAll && m_pkgElem != null) {
			// isInGenericPackage == true
			ArrayList<Transition_c> tran_set = new ArrayList<Transition_c>();
		...
		if (m_csm_transitions != null)
			m_csm_transitions = null;
		if (parseAll && m_ss_set != null) {
			m_csm_transitions = Transition_c.getManySM_TXNsOnR505(StateMachine_c.getManySM_SMsOnR517(
					ClassStateMachine_c.getManySM_ASMsOnR519(ModelClass_c.getManyO_OBJsOnR2(m_ss_set))));
		} else if (parseAll && m_pkgElem != null) {
			// isInGenericPackage == true
			ArrayList<Transition_c> tran_set = new ArrayList<Transition_c>();
...
>>>>>>> refs/remotes/travis/7738_automated_tests_mars
