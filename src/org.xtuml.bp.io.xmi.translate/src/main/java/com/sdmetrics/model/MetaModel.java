...
public class MetaModel implements Iterable<MetaModelElement> {
	...
	private class MetaModelParser extends SAXHandler {
		...
		private void handleMetaModelElement(Attributes attrs)
			...
			String mapping = attrs.getValue(ATTR_MAPPING);

			currentMME = new MetaModelElement(typeName, mapping, parentElement);
		...
		private void handleAttributeDefinition(Attributes attrs)
			...
			currentMapping = attrs.getValue(ATTR_MAPPING);
			...
			currentMME.addAttribute(currentAttributeName, currentMapping, isRefAttribute,
					isSetAttribute);
		}
