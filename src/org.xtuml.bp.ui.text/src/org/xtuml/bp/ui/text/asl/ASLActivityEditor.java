...
public class ASLActivityEditor extends ASLEditor {
	...
	/**
	 * Whether this editor has been closed, which an associated parse thread needs
	 * to know when finishing its wait to run, as it should abort at that time if
	 * this is true.
	 */
	private boolean disposed = false;

	// TODO private static final String m_parseThreadName = "org.xtuml.bp.ui.text.parseThread."; //$NON-NLS-1$
	// TODO private volatile TolerantThread parseThread = null;
	private static int HANGON_DURATION_original_value = 500, // in milli seconds
			HANGON_DURATION = HANGON_DURATION_original_value;
	// TODO public static String getParseThreadName () { return m_parseThreadName; }

	/*
	 * private synchronized void accessParseThread(Runnable r, String name) { if
	 * (parseThread != null && parseThread.isAlive()){ parseThread.interrupt(); try
	 * { parseThread.join(); } catch (InterruptedException e) {} }
	 * 
	 * // start a new parse thread parseThread = new TolerantThread(r, name,
	 * parseThread); parseThread.start(); }
	 */

	// protected static class ParseRunnable implements Runnable {
	// public ASLActivityEditorInput m_ae_input;
	...
	// */
	// private ASLActivityEditor forEditor;
	//
	// /**
	// * Constructor.
	// *
	...
	// */
	// public ParseRunnable(ASLActivityEditor forEditor)
	// {
	...
	// } else {
	// String errorMsg = "Internal parser error. Token stream exception in parser.
	// OAL in this action home caused an exception in the parser."; //$NON-NLS-1$
	// ActivityProblem ap =
	// new ActivityProblem(
	// errorMsg,
	...
	// parser.m_oal_context.m_act_id);
	// } catch(Throwable t){
	// String errorMsg = "Internal parser error. Parsing thread interrupted
	// pre-maturely. OAL in this action home caused an exception in the parser.";
	// //$NON-NLS-1$
	// Block_c.Clearcurrentscope(modelRoot,
	// parser.m_oal_context.m_act_id);
	//
	...
	//
	// m_myAnnotationModel.acceptProblem(ap);
	// //This throwable catches all the un-checked exceptions that occur in the
	// thread, and logs them
	// //appropriately.
	...
	// {
	// private ParseRunnable fRunnable = new ParseRunnable(ASLActivityEditor.this);
	//
	...
	// NonRootModelElement modelElement = ((ASLActivityEditorInput)
	...
	// fRunnable.m_ae_input = (ASLActivityEditorInput) input;
	...
	// accessParseThread(fRunnable, getParseThreadName() +
	// fRunnable.m_ae_input.getName());
	// Thread.yield();
	// }
	// }
	// }
	...
	// if
	// (!Pref_c.Getboolean(BridgePointPreferencesStore.ENABLE_PARSE_ON_ACTIVITY_EDITS))
	// {
	// return;
	// }
	//
	...
	protected void doSetInput(IEditorInput input) throws CoreException {
		...
		/*
		 * TODO - do we need this?? if(document != null){ listener = new Listener();
	...
	public void dispose() {
		...
		/*
		 * TODO - don't need if (parseThread != null && parseThread.isAlive()) { // if
		 * the parse thread is waiting to run, end the wait, so it can // notice this
		 * editor is disposed and die parseThread.interrupt(); }
		 */
	...
	/**
	 * See HANGON_DURATION field. Tests might want to lower the value of this field
	 * to be able to complete faster.
	 */
