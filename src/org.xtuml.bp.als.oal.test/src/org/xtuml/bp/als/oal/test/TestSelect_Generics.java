...
import org.xtuml.bp.core.SelectedReference_c;
...
public class TestSelect_Generics extends TestCase {
    ...
    static public void badSelectValidate(String[] vars, int numStmt,
        ...
        OalParserTest_Generics.badRelationshipValidate(vars, numStmt, numSel,
                numVal);
        ChainLink_c[] cl = ChainLink_c
                .ChainLinkInstances(OalParserTest_Generics.modelRoot);
...
public void goodSelectValidate(
	...
	if (from_inst && where) {
		...
		SelectFromInstancesWhere_c sel[] = SelectFromInstancesWhere_c.SelectFromInstancesWhereInstances(OalParserTest_Generics.modelRoot);
        ...
        SelectedReference_c[] sr = SelectedReference_c.SelectedReferenceInstances(OalParserTest_Generics.modelRoot);
		...
		if (numSrs == 1) {
          ...
          assertEquals(val[indexBO].getValue_id(), bin[0].getValue_id());
            ...
            if ( testAttrValInst == true ) {
                OalParserTest_Generics.verifyBinaryOperation(bin[0], "==", av[0].getValue_id(), lin[0].getValue_id());//$NON-NLS-1$
            } else {
                OalParserTest_Generics.verifyBinaryOperation(bin[0], "==", sr[0].getValue_id(), lin[0].getValue_id());//$NON-NLS-1$
            }
		  } else {
			InstanceReference_c[] ir_val = InstanceReference_c.InstanceReferenceInstances(OalParserTest_Generics.modelRoot);
			assertEquals(1, ir_val.length);
			assertEquals(ir_val[0].getValue_id(), val[indexLI].getValue_id());
            if ( testAttrValInst == true ) {
                OalParserTest_Generics.verifyBinaryOperation(bin[0], "==", av[0].getValue_id(), ir_val[0].getValue_id());//$NON-NLS-1$
            } else {
                OalParserTest_Generics.verifyBinaryOperation(bin[0], "==", sr[0].getValue_id(), ir_val[0].getValue_id());//$NON-NLS-1$
            }
		  }
		}
		...
		SelectFromInstances_c sel[] = SelectFromInstances_c.SelectFromInstancesInstances(OalParserTest_Generics.modelRoot);
		assertEquals(numSel, sel.length);
		// only verify the last one
		assertEquals(st[numStmt - 1].getStatement_id(), sel[numSel - 1].getStatement_id());
		assertEquals(var[0].getVar_id(), sel[numSel - 1].getVar_id());
...
private void validateNLinkChain(Select_c sel, int num, String [] objs, int relNum, String[] phrases) {
	...
	for ( int i =0; i < num; ++i) {
		ChainLink_c[] cl2 = ChainLink_c.ChainLinkInstances(OalParserTest_Generics.modelRoot, new ChainLink_test1_c(cl.getNext_link_id()));
		if ( i == num - 1 ) {
...
@Test
	...
	OalParserTest_Generics.badRelationshipValidate(var_list, 0, 0, 0);
	Value_c[] val = Value_c.ValueInstances(OalParserTest_Generics.modelRoot);
