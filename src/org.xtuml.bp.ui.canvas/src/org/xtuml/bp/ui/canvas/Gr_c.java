...
public class Gr_c {
	...
	public static void Drawtext(final GCDelegate Context, final int Justified_to, final String Text,
			final int Text_style, final int X, final int Y) {
		...
		if (Style_c.Underlined == Text_style) {
			...
			if (Justified_to == Justification_c.Center_in_X) {
				for (int i = 0; i < toPrint.length; i++) {
					...
					Context.drawText(toPrint[i], (int) ((X - (width / 2)) * m_ZoomFactor),
							(int) ((Y + yOffset) * m_ZoomFactor), true);
				...
				for (int i = 0; i < toPrint.length; i++) {
					int width = Scale(Context.textExtent(toPrint[i]).x);
					Context.drawText(toPrint[i], (int) ((X - width) * m_ZoomFactor),
							(int) ((Y + yOffset) * m_ZoomFactor), true);
					yOffset = yOffset + Scale(Context.textExtent(toPrint[i]).y);
				}
			} else {
	...
	public static void Drawrect(final GCDelegate Context, boolean filled, final int H, final int W, final int X,
			final int Y) {
		if (filled) {
			Context.fillRectangle((int) (X * m_ZoomFactor), (int) (Y * m_ZoomFactor), (int) (W * m_ZoomFactor),
					(int) (H * m_ZoomFactor));
		}
		Context.drawRectangle((int) (X * m_ZoomFactor), (int) (Y * m_ZoomFactor), (int) (W * m_ZoomFactor),
				(int) (H * m_ZoomFactor));
	} // End drawRect

	public static void Drawnote(final GCDelegate Context, final int H, final int W, final int X, final int Y) {
		Rectangle scaledRect = new Rectangle((int) (X * m_ZoomFactor), (int) (Y * m_ZoomFactor),
				(int) (W * m_ZoomFactor), (int) (H * m_ZoomFactor));
		Rectangle cornerRect = new Rectangle((int) (scaledRect.x + scaledRect.width - 16 * m_ZoomFactor), scaledRect.y,
				(int) m_ZoomFactor * 16, (int) m_ZoomFactor * 16);
		// fill full poly
		int[] poly = new int[] { scaledRect.x, scaledRect.y, cornerRect.x, cornerRect.y,
				cornerRect.x + cornerRect.width, cornerRect.y + cornerRect.height, scaledRect.x + scaledRect.width,
				scaledRect.y + scaledRect.height, scaledRect.x, scaledRect.y + scaledRect.height, scaledRect.x,
				scaledRect.y };
		Context.fillPolygon(poly);
		// draw full ploy
		Context.drawPolygon(poly);
		// draw fold (NOTE: no idea why the +1 is required in the first line)
		Context.drawLine(cornerRect.x, cornerRect.y, cornerRect.x + 1, cornerRect.y + cornerRect.height);
		Context.drawLine(cornerRect.x, cornerRect.y + cornerRect.height, cornerRect.x + cornerRect.width,
				cornerRect.y + cornerRect.height);
	} // End drawRect

	public static void Clipto(final GCDelegate Context, final int H, final int W, final int X, final int Y) {
		Context.setClipping((int) (X * m_ZoomFactor), (int) (Y * m_ZoomFactor), (int) (W * m_ZoomFactor),
				(int) (H * m_ZoomFactor));
	} // End clipTo
	...
	public static void Drawline(final GCDelegate Context, final int X, final int X2, final int Y, final int Y2) {
		Context.drawLine((int) (X * m_ZoomFactor), (int) (Y * m_ZoomFactor), (int) (X2 * m_ZoomFactor),
				(int) (Y2 * m_ZoomFactor));
	} // End drawLine
	...
	public static void Drawroundrect(final GCDelegate Context, final int H, final int W, final int X, final int Y) {
		int curveRadius = 25;
		Context.fillRoundRectangle((int) (X * m_ZoomFactor), (int) (Y * m_ZoomFactor), (int) (W * m_ZoomFactor),
				(int) (H * m_ZoomFactor), (int) (curveRadius * m_ZoomFactor), (int) (curveRadius * m_ZoomFactor));
		Context.drawRoundRectangle((int) (X * m_ZoomFactor), (int) (Y * m_ZoomFactor), (int) (W * m_ZoomFactor),
				(int) (H * m_ZoomFactor), (int) (curveRadius * m_ZoomFactor), (int) (curveRadius * m_ZoomFactor));
	} // End drawRoundRect

	public static void Drawfolder(final GCDelegate Context, final int H, final int T, final int W, final int X,
			final int Y) {
		// Tab
		Context.fillRectangle((int) (X * m_ZoomFactor), (int) (Y * m_ZoomFactor), (int) ((3 * T) * m_ZoomFactor),
				(int) ((T + 2) * m_ZoomFactor));
		Context.drawRectangle((int) (X * m_ZoomFactor), (int) (Y * m_ZoomFactor), (int) ((3 * T) * m_ZoomFactor),
				(int) ((T + 2) * m_ZoomFactor));
		// Main Area
		Context.fillRectangle((int) (X * m_ZoomFactor), (int) ((T + Y) * m_ZoomFactor), (int) (W * m_ZoomFactor),
				(int) ((H - T) * m_ZoomFactor));
		Context.drawRectangle((int) (X * m_ZoomFactor), (int) ((T + Y) * m_ZoomFactor), (int) (W * m_ZoomFactor),
				(int) ((H - T) * m_ZoomFactor));
	} // End drawFolder
	...
	private static Point rotateAboutOrigin(Point start, double theta) {
		...
		// Principles of Interactive Computer Graphics, p54, Newman and Sproull ISBN
		// 0-07-066455-2 7th Printing (1982).
		return new Point((int) Math.round((double) start.x * Math.cos(theta) + (double) start.y * Math.sin(theta)),
	...
	public static int Getxintersect(final int X1, final int X2, final int X3, final int X4, final int Y1, final int Y2,
			final int Y3, final int Y4) {
		Point result = intersect(new Point(X1, Y1), new Point(X2 - X1, Y2 - Y1), new Point(X3, Y3),
				new Point(X4 - X3, Y4 - Y3));
		if (result != null) {
			return result.x;
		} else {
			return 0;
	...
	public static int Getyintersect(final int X1, final int X2, final int X3, final int X4, final int Y1, final int Y2,
			final int Y3, final int Y4) {
		Point result = intersect(new Point(X1, Y1), new Point(X2 - X1, Y2 - Y1), new Point(X3, Y3),
				new Point(X4 - X3, Y4 - Y3));
		if (result != null) {
	...
	private static Point intersect(Point v1, Point v2, Point v3, Point v4) {
		...
		if (Math.abs(delta) > 0.0000001d) {
			float mu = (((float) v3.x - (float) v1.x) * (float) v4.y - ((float) v3.y - (float) v1.y) * (float) v4.x)
					/ delta;
	...
	public static String Wraptextto(final GCDelegate Context, final String Input, final int Width) {
		...
		for (int i = 0; i < lines.length; i++) {
			...
			while (currentWord < wordCount) {
				...
				int wordWidth = Gr_c.Gettextextent(Axis_c.X, Context, words[currentWord]);
				if (wordWidth > Width) {
					...
					while (lineWidth <= Width && currentWord < wordCount) {
						...
						if (lineWidth <= Width) {
							...
							if (currentWord < wordCount) {
								...
								lineWidth = Gr_c.Gettextextent(Axis_c.X, Context, line + " " + words[currentWord]);//$NON-NLS-1$
							}
						}
						atLineStart = false;
					}
	...
	public static String Fittextto(final GCDelegate Context, boolean elideAtEnd, final String Input, final int Width) {
		...
		for (int i = 0; i < toFit.length; i++) {
			...
			if (x > Width) {
				...
				while (Gr_c.Gettextextent(Axis_c.X, Context, result[i]) > fitLen && len > 1) {
					if (elideAtEnd)
						result[i] = result[i].substring(0, --len);
					else
						result[i] = result[i].substring(1, len--);
				}
				if (elideAtEnd)
					result[i] += ellipsis;
				else
	...
	public static void Drawarrow(final GCDelegate Context, boolean filled, final float Rotation, final int X,
			final int Y) {
		if (openArrow == null) {
			openArrow = new int[6];
			openArrow[0] = -6;
	...
	public static void Drawactor(final GCDelegate Context, final int height, final int width, final int X,
			final int Y) {
		// draw the head
		int radius = width / 8;
	...
	public static void Drawcircle(final GCDelegate Context, boolean filled, final int Radius, final int X,
			final int Y) {
		int w = Radius * 2;
		int h = Radius * 2;
		int x = X - Radius;
		...
		if (filled) {
			...
			Context.fillOval((int) (x * m_ZoomFactor), (int) (y * m_ZoomFactor), (int) (w * m_ZoomFactor),
					(int) (h * m_ZoomFactor));
			Context.setBackground(current);
		} else {
			Context.drawOval((int) (x * m_ZoomFactor), (int) (y * m_ZoomFactor), (int) (w * m_ZoomFactor),
					(int) (h * m_ZoomFactor));
		}
	} // End drawFilledCircle

	public static void Drawsemicircle(final GCDelegate Context, boolean filled, final int Radius, float Rotation,
		...
		Context.drawArc((int) (x * m_ZoomFactor), (int) (y * m_ZoomFactor), (int) (w * m_ZoomFactor),
				(int) (h * m_ZoomFactor), (int) startAngle, 180);
	} // End drawFilledCircle
	...
	public static Cursor getOpenHand() {
		if (openHand == null) {
			openHand = new Cursor(Display.getDefault(), CorePlugin.getImageDescriptor("hand.gif").getImageData(), //$NON-NLS-1$
	...
	public static Cursor getClosedHand() {
		if (closedHand == null) {
			closedHand = new Cursor(Display.getDefault(),
	...
	private static Cursor getPencil() {
		if (pencil == null) {
			pencil = new Cursor(Display.getDefault(), CorePlugin.getImageDescriptor("pencil.gif").getImageData(), //$NON-NLS-1$
	...
	private static Cursor getNesw() {
		if (nesw == null) {
			nesw = new Cursor(Display.getDefault(), CorePlugin.getImageDescriptor("nesw_arrows.gif").getImageData(), //$NON-NLS-1$
	...
	private static Cursor getNwse() {
		if (nwse == null) {
			nwse = new Cursor(Display.getDefault(), CorePlugin.getImageDescriptor("nwse_arrows.gif").getImageData(), //$NON-NLS-1$
	...
	private static Cursor getWaypoint() {
		if (waypoint == null) {
			waypoint = new Cursor(Display.getDefault(),
	...
	public static void Drawflexhandles(final GCDelegate Context, final int Radius, final int X, final int Y) {
		...
		Context.drawOval((int) (offsetX * m_ZoomFactor), (int) (offsetY * m_ZoomFactor),
				(int) (Radius * m_ZoomFactor) * 2, (int) (Radius * m_ZoomFactor) * 2);
		// Restore line width
		Context.setLineWidth(lineWidth);
	} // End drawFlexHandles
	...
	public static GCDelegate Getcontext() {
		...
		String prefFont = JFacePreferences.getPreferenceStore().getString("org.xtuml.bp.canvas.font");//$NON-NLS-1$
	...
	public static boolean Isover(final int Tolerance, final int X, final int X1, final int X2, final int Y,
	...
	/**
	 * Draws the image of the given name within the given graphics context at the
	 * given diagram coordinates, which are converted to the canvas' space. The
	 * image is scaled according to the diagram's zoom factor.
	 */
	public static void Drawimage(GCDelegate Context, String name, int x, int y) {
		...
		Context.drawImage(image, 0, 0, bounds.width, bounds.height, (int) (x * m_ZoomFactor), (int) (y * m_ZoomFactor),
				(int) (bounds.width * m_ZoomFactor), (int) (bounds.height * m_ZoomFactor));
	}
