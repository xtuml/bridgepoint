...
idee-da7a-c0c0-c0c0-idididididid

Where:
- The first two groups are a fixed stamp analogous to ba5e-da7a for built in
  data types.
- The third and fourth groups shall be the component id rendered as 8 hexadecimal
  digits and split between the two groups, high word first, lower word second.
- The final group shall be the unique identifier value rendered as 12 hexadecimal
  digits.

6.1.2 Externally originated UUID values

A private static map shall be maintained in VerifierInvocationHandler. When an
externally created UUID is passed to Verifier, an internally generated unique_id
will be created for it. The UUID value will be added to the static map using the
...
defined in 6.1, the static map is checked to see if the UUID has been seen before.
...
On termination of a session, the static map shall be cleared.
...
A total of 12 tests are therefore required to fully test the proposed functionality.
