...
4.3  User-selected preferences shall be applied to the stored values when the
  preference dialog is dismissed with OK.  
...
5.2  Update ```BridgePointPreferenceStore.java``` to stop calling ```store.setDefault()```
  in the ```loadModel()``` function.  When we try to set the default value here
  it was overwriting the default set in the INI file.  The code is changed to 
  remove these calls and just use the INI settings.    
...
5.3.2  Make sure each page has a ```performDefaults()``` function that resets
  local copy of the BridgePoint preferences to the default values then updates
  the current pages UI with the new settings.     
5.3.3  Where possible, refactor the code the break out preference setting to a 
  single routine that is callable from multiple places.  
5.3.4  The author discovered that when closing the preferences dialog inside
  ...
  Each of these ```performOk()'s``` may cause the store to be updated.  So, out
  code needs to make sure the local copy of the preferences model is up to date 
  before we modify and save it.  
...
7.1  Test preference change in INI  
  ...
  - __R__ Verify the setting "Allow operations inside where clauses of select statements" is unchecked
  - __R__ Verify the setting "Allow lossy assignment of real to int" is marked Yes
  - Select OK
  - Close BridgePoint
  - Open ```bp.pkg/plugin_customization.ini```
  - Change the setting ```com.mentor.nucleus.bp.core/bridgepoint_prefs_allow_operations_in_where=false``` to true
  - Change the setting ```com.mentor.nucleus.bp.core/bridgepoint_prefs_allow_real_to_int_coercion=always``` to never
  - Start BridgePoint
  - Open ```Window > Preferences > xtUML > Action Language```
  - __R__ Verify the setting "Allow operations inside where clauses of select statements" is checked
  - __R__ Verify the setting "Allow lossy assignment of real to int" is marked No
  - Select OK
  - Close BridgePoint
  - Restore the original values to the changed settings in ```bp.pkg/plugin_customization.ini```
