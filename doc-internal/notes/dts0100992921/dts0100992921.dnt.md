...
and to create themif they are not there.
...
source a small dll that exposes one member function, 'getMessage'. This returns
the string "Hello JNI.". The dll is built automatically by CDT. The project also
declares some client java classes which load the dll, call the function and
return the method to a VIEC compatible realized java class ('Realized.java).
The test model makes the appropriate calls and logs the resulting message.
